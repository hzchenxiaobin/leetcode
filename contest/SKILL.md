...
周赛每场固定 4 题（Q1 简单 → Q4 困难），题解**不必四题全写**，按价值取舍。

### 1.1 取舍原则

| 原则 | 说明 |
|------|------|
| **Q1 一般跳过** | Q1 通常是签到题（按位求和、简单模拟等），套路价值低，除非有巧妙观察否则不写 |
| **Q2/Q3 重点** | 中等题套路代表性强，常考滑动窗口、DP、贪心、二分，优先写 |
| **Q4 视情况** | 困难题若思路精妙（构造、高级数据结构、最短路变体）必写；若纯套路堆砌可略 |
| **不会的题必写** | 比赛时没做出来或思路卡壳的题必须补全题解，这是复盘的核心价值 |
| **题目不重复** | 同一道题在 daily 与 contest 中只出现一次；选题前先查 `leetcode/contest/` 与 `leetcode/daily/` 已有文件 |

> 💡 **一句话总结**：周赛题解的本质是「赛后复盘 + 套路沉淀」，只写有学习价值的题，每题都要能提炼一个可迁移的模板或观察。

### 1.2 每场周赛的建议产出

| 位置 | 难度 | 建议 |
|------|------|------|
| Q1 | 简单 | 跳过（签到题），除非有非平凡观察 |
| Q2 | 中等 | 写（套路题，必出题解） |
| Q3 | 中等/困难 | 写（常是分场胜负的关键题） |
| Q4 | 困难 | 比赛做出来则略，没做出来必写（复盘重点） |
