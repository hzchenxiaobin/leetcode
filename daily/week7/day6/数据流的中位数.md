...
> 💡 与 [Week7 Day6 全链路 Profiling](../../aiinfra/daily/week7/day6/README.md) 中的 **P50 延迟统计**同构——P50 就是中位数，用堆维护可以 O(log N) 插入 + O(1) 查中位数。Profiling 脚本中 `statistics.median()` 对应 `findMedian()`，`PhaseTimer` 的 `times` 列表对应数据流。
...
## 7. 同类练习题
- [295. 数据流的中位数](https://leetcode.cn/problems/find-median-from-data-stream/)（[题解](数据流的中位数.md)）：双堆
