...
> 💡 与 [Week7 Day2 完整调度器](../../aiinfra/daily/week7/day2/README.md) 的资源预算同构——冷却时间 `n` 类比 `token_budget` 的约束：不能在一轮内把同类任务全做完，必须"穿插"其他任务。调度器在 token_budget 不足时让请求等待下一轮，任务调度器在冷却时间内执行其他任务或空闲。贪心策略（先排最高频，空位填低频）对应调度器的"优先级高的先分配资源，剩余预算给低优先级"。
