...
def build_nav(current_slug: Optional[str], problems: List[Dict]) -> str:
    ...
    lines.append(f'<a class="{overview_class}" href="/leetcode/index.html">📌 题解列表</a>')

    lines.append('<div class="nav-section-title">题目</div>')
    for p in problems:
        cls = "nav-link active" if current_slug == p["slug"] else "nav-link"
        lines.append(f'<a class="{cls}" href="/leetcode/problems/{p["slug"]}.html">{p["title"]}</a>')
...
{nav_html}
        ...
        <main class="main-content">
            <div class="page-header">
                ...
                <a class="back-link" href="/leetcode/index.html">← 返回题解列表</a>
            </div>
            <article class="content" id="content"></article>
            <div class="day-nav-bottom"><a class="back-link" href="/leetcode/index.html">← 返回题解列表</a></div>
        </main>
    </div>
...
def build_website(leetcode_dir: Path, output_dir: Path) -> None:
    ...
    for md_file in md_files:
        ...
        # Markdown references images as "images/xxx.svg"; convert to absolute path
        markdown_text = markdown_text.replace("](images/", "](/leetcode/images/")
    ...
    for p in problems:
        cards_html += (
            f'<a class="day-card" href="/leetcode/problems/{p["slug"]}.html">\n'
    ...
    overview_html = page_template(
        title="LeetCode 题解",
        nav_html=build_nav(current_slug=None, problems=problems),
    ...
    for p in problems:
        html = page_template(
            title=p["title"],
            nav_html=build_nav(current_slug=p["slug"], problems=problems),
            markdown=p["markdown"],
        )
