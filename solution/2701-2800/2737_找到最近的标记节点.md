...
- **题目名称**：找到最近的标记节点
- **链接**：[2737. 找到最近的标记节点 🔒](https://leetcode.cn/problems/find-the-closest-marked-node/)
- **难度**：中等
...
给定一个含 `n` 个节点的**带权无向图**，节点编号 `0` 到 `n-1`。边以数组 `edges` 给出，`edges[i] = [u, v, w]` 表示 `u` 与 `v` 之间有一条权值为 `w` 的边。另给一个起点 `s` 和一个标记节点数组 `marked`。
