...
- **题目名称**：数组的上界
- **链接**：[2774. 数组的上界](https://leetcode.cn/problems/array-upper-bound/)
- **难度**：简单
...
给定一个**升序数组**，请你在 `Array.prototype` 上挂载一个方法 `upperBound(target)`，返回 `target` 在数组中出现的**最右位置（下标）**；若 `target` 不在数组中则返回 `-1`。
