...
- **题目名称**：美丽下标对的数目
- **链接**：[2748. 美丽下标对的数目](https://leetcode.cn/problems/number-of-beautiful-pairs/)
- **难度**：简单
- **标签**：数组、哈希表、数学、计数、数论

## 1. 题目概述

给你一个下标从 **0** 开始的整数数组 `nums`。如果下标对 `i`、`j` 满足 `0 ≤ i < j < nums.length`，且 `nums[i]` 的**第一个数字**和 `nums[j]` 的**最后一个数字** **互质**，则认为 `nums[i]` 和 `nums[j]` 是一组**美丽下标对**。
