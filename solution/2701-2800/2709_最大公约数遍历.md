...
- **题目名称**：最大公约数遍历
- **链接**：[2709. 最大公约数遍历](https://leetcode.cn/problems/greatest-common-divisor-traversal/)
- **难度**：困难
- **标签**：并查集、数组、数学、数论、质因数分解、欧几里得算法

## 1. 题目概述

给你一个下标从 **0** 开始的整数数组 `nums`，你可以在一些下标之间遍历。对于两个下标 `i` 和 `j`（`i != j`），当且仅当 $\gcd(\text{nums}[i], \text{nums}[j]) > 1$ 时，我们可以在两个下标之间通行，其中 $\gcd$ 是两个数的**最大公约数**。
