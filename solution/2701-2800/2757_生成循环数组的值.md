...
- **题目名称**：生成循环数组的值
- **链接**：[2757. Generate Circular Array Values](https://leetcode.cn/problems/generate-circular-array-values/)
- **难度**：中等
...
给定一个**循环数组** `arr`（即 `arr[n-1]` 的下一个元素是 `arr[0]`，`arr[0]` 的上一个元素是 `arr[n-1]`，`n = arr.length`），一个整数数组 `steps`，以及一个起始下标 `startIndex`。实现一个**生成器函数** `cycleGenerator`，它返回一个生成器对象，按如下规则逐个产出 `arr` 中的值：
