...
- **题目名称**：找到具有最小和的树的层数
- **链接**：[3157. 找到具有最小和的树的层数](https://leetcode.cn/problems/find-the-level-of-tree-with-minimum-sum/)
- **难度**：中等
...
给你一棵二叉树的根节点 `root`。设根节点位于二叉树的**第 1 层**，根节点的子节点位于第 2 层，依此类推。返回节点值总和**最小**的那一层的层号 `x`；若有多层的总和相同，返回其中**最小**的层号 `x`。
