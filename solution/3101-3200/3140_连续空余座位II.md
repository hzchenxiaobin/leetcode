...
- **题目名称**：连续空余座位 II
- **链接**：[3140. 连续空余座位 II](https://leetcode.cn/problems/consecutive-available-seats-ii/)
- **难度**：中等
...
给定 `Cinema` 表，每行记录一个座位的占用情况：座位号 `seat_id` 与空闲标志 `free`（`1` 表示空余，`0` 表示被占）。编写 SQL 查询，找出影厅里**最长的连续空余座位段**，输出该段的首座位号 `first_seat_id`、尾座位号 `last_seat_id` 与段长 `consecutive_seats_len`，按 `first_seat_id` **升序**返回。
