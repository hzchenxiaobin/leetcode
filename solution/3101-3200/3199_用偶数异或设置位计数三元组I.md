...
- **题目名称**：用偶数异或设置位计数三元组 I
- **链接**：[3199. 用偶数异或设置位计数三元组 I](https://leetcode.cn/problems/count-triplets-with-even-xor-set-bits-i/)
- **难度**：简单
- **标签**：位运算、数组
...
给你三个整数数组 `a`、`b`、`c`。对每个三元组 $(i, j, k)$（$0 \le i < |a|$，$0 \le j < |b|$，$0 \le k < |c|$）计算 $a[i] \oplus b[j] \oplus c[k]$（$\oplus$ 为按位异或），请统计其**置位位数**（二进制表示中 1 的个数，即 popcount）为**偶数**（0 也是偶数）的三元组数目。
