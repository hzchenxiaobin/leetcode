...
- **题目名称**：通过添加或删除结尾字符来同化字符串
- **链接**：[3135. 通过添加或删除结尾字符来同化字符串](https://leetcode.cn/problems/equalize-strings-by-adding-or-removing-characters-at-ends/)
- **难度**：中等
...
给定两个字符串 `initial` 和 `target`（均只含小写英文字母），要求通过若干次操作把 `initial` 变成 `target`。每次操作只能是以下四种之一：
