...
- **题目名称**：找到最常见的回答
- **链接**：[3527. 找到最常见的回答](https://leetcode.cn/problems/find-the-most-common-response/)
- **难度**：中等
- **标签**：数组、哈希表、字符串、计数

## 1. 题目概述

给你一个二维字符串数组 `responses`，其中 `responses[i]` 是一个字符串数组，表示第 $i$ 天调查的回答结果。请返回在对每个 `responses[i]` 中的回答**去重**后，所有天数中**最常见**的回答。如果有多个回答出现频率相同，则返回**字典序最小**的那个回答。
