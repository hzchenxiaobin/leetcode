...
- **题目名称**：单位转换 II
- **链接**：[3535. 单位转换 II](https://leetcode.cn/problems/unit-conversion-ii/)
- **难度**：中等
...
有 $n$ 种单位，编号 $0$ 到 $n-1$。给定长度为 $n-1$ 的二维整数数组 `conversions`，其中 `conversions[i] = [sourceUnit_i, targetUnit_i, conversionFactor_i]`，表示 **1 个 `sourceUnit_i` 类型的单位等于 `conversionFactor_i` 个 `targetUnit_i` 类型的单位**。
