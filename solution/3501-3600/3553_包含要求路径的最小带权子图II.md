...
- **题目名称**：包含要求路径的最小带权子图 II
- **链接**：[3553. 包含要求路径的最小带权子图 II](https://leetcode.cn/problems/minimum-weighted-subgraph-with-the-required-paths-ii/)
- **难度**：困难
- **标签**：树、最近公共祖先、倍增、位运算、深度优先搜索

## 1. 题目概述

给你一棵 **无向带权树**，共 $n$ 个节点（编号 $0$ 到 $n-1$），由长度为 $n-1$ 的二维数组 `edges` 描述，`edges[i] = [ui, vi, wi]` 表示节点 $u_i$ 与 $v_i$ 之间有一条权重为 $w_i$ 的无向边。
