...
1. **[3504. 子字符串连接后的最长回文串 II](https://leetcode.cn/problems/longest-palindrome-after-substring-concatenation-ii/)**：题面相同，仅把数据范围放宽到 $n, m \le 100$。本文的 $O\big(nm \cdot \min(n,m)\big) \approx 10^6$ 解法**原样通过**，无需任何改动——这正是结构化枚举优于暴力的意义所在（暴力 $O(n^2m^2(n+m))$ 在 $100$ 的规模下约 $1.2 \times 10^{10}$，必然超时）。
