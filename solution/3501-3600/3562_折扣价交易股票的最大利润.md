...
- **题目名称**：折扣价交易股票的最大利润
- **链接**：[3562. 折扣价交易股票的最大利润](https://leetcode.cn/problems/maximum-profit-from-trading-stocks-with-discounts/)
- **难度**：困难
- **标签**：树, 深度优先搜索, 数组, 动态规划

## 1. 题目概述

公司有 $n$ 位员工，编号 $1 \sim n$，员工 $1$ 是 CEO。层级关系构成一棵**以 1 为根的树**：`hierarchy[i] = [u, v]` 表示 $u$ 是 $v$ 的直属上司。
