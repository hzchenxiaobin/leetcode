...
- **题目名称**：带权树中的最短路径
- **链接**：[3515. 带权树中的最短路径](https://leetcode.cn/problems/shortest-path-in-a-weighted-tree/)
- **难度**：困难
- **标签**：树、深度优先搜索、树状数组、线段树、数组

## 1. 题目概述

给你一个整数 `n` 和一棵以节点 1 为根的无向带权树，共 `n` 个节点（编号 1 到 n），由长度为 `n - 1` 的二维数组 `edges` 描述，`edges[i] = [ui, vi, wi]` 表示节点 `ui` 与 `vi` 之间有一条权重为 `wi` 的无向边。
