...
- **题目名称**：求出数组的 X 值 II
- **链接**：[3525. 求出数组的 X 值 II](https://leetcode.cn/problems/find-x-value-of-array-ii/)
- **难度**：困难
- **标签**：线段树、数组、数学

## 1. 题目概述

给定一个由**正整数**组成的数组 `nums` 和一个**正整数** `k`，再给定二维数组 `queries`，其中 `queries[i] = [index_i, value_i, start_i, x_i]`。
