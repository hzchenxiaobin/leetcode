...
- **题目名称**：执行指令后的得分
- **链接**：[3522. Calculate Score After Performing Instructions](https://leetcode.cn/problems/calculate-score-after-performing-instructions/)
- **难度**：中等
- **标签**：数组、哈希表、字符串、模拟

## 1. 题目概述

给定两个长度均为 $n$ 的数组 `instructions` 与 `values`，按以下规则模拟一个过程：
