...
- **题目名称**：最大化交错和为 K 的子序列乘积
- **链接**：[3509. 最大化交错和为 K 的子序列乘积](https://leetcode.cn/problems/maximum-product-of-subsequences-with-an-alternating-sum-equal-to-k/)
- **难度**：困难
- **标签**：数组、哈希表、动态规划

## 1. 题目概述

给定整数数组 `nums` 和两个整数 `k` 与 `limit`，找一个**非空子序列**，满足：
