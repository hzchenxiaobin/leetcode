...
- **题目名称**：可以输入的最大单词数
- **链接**：[1935. 可以输入的最大单词数](https://leetcode.cn/problems/maximum-number-of-words-you-can-type/)
- **难度**：简单
- **标签**：哈希表、字符串、模拟

## 1. 题目概述

键盘有些字母键失灵，其余键正常。给定由**单个空格**分隔的字符串 `text`（不含前导/尾随空格），以及由**互不相同**小写字母组成的 `brokenLetters`，返回 `text` 中能**完全输入**的单词数目——即不包含任何破损字母的单词个数。
