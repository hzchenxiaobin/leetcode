...
- **题目名称**：规定时间内到达终点的最小花费
- **链接**：[1928. 规定时间内到达终点的最小花费](https://leetcode.cn/problems/minimum-cost-to-reach-destination-in-time/)
- **难度**：困难
- **标签**：图、最短路径、Dijkstra、动态规划

## 1. 题目概述

一个国家有 $n$ 个城市（编号 $0$ 到 $n-1$），城市间由双向道路连接，道路由 `edges = [[x, y, time], ...]` 表示，`time` 为通行分钟数。每次经过一个城市需付通行费 `passingFees[j]`。
