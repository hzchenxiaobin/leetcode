...
- **题目名称**：感染 K 种病毒所需的最短时间
- **链接**：[1956. 感染 K 种病毒所需的最短时间](https://leetcode.cn/problems/minimum-time-for-k-virus-variants-to-spread/)
- **难度**：困难
- **标签**：几何、二分查找、枚举、坐标变换、有序集合、滑动窗口

## 1. 题目概述

在无限大的二维平面上有 $n$ 种**不同**的病毒。给定二维数组 `points`，第 $i$ 项 `points[i] = [x_i, y_i]` 表示第 $0$ 天有一种病毒位于点 $(x_i, y_i)$。注意初始状态下，可能有**多种**病毒在**同一点**上。
