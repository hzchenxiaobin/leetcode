...
- **题目名称**：丢失信息的雇员
- **链接**：[1965. 丢失信息的雇员](https://leetcode.cn/problems/employees-with-missing-information/)
- **难度**：简单
- **标签**：数据库、SQL、`UNION ALL`、`GROUP BY`、`HAVING`、对称差（symmetric difference）、`LEFT JOIN`、`IS NULL`、`NOT IN` 的 NULL 陷阱

## 1. 题目概述

给定 `Employees`（雇员姓名）和 `Salaries`（雇员薪水）两张表，编写 SQL 查询，找出所有**丢失信息**的雇员 `employee_id`。当满足下列条件**之一**即视为信息丢失：
