...
- **题目名称**：游戏中弱角色的数量
- **链接**：[1996. 游戏中弱角色的数量](https://leetcode.cn/problems/the-number-of-weak-characters-in-the-game/)
- **难度**：中等
- **标签**：数组、排序、贪心、单调维护

## 1. 题目概述

你正在参加一个多角色游戏，每个角色都有两个主要属性：**攻击** 和 **防御**。给定二维整数数组 `properties`，其中 `properties[i] = [attack_i, defense_i]` 表示第 $i$ 个角色的属性。
