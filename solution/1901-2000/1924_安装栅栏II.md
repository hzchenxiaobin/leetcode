...
- **题目名称**：安装栅栏II
- **链接**：[1924. 安装栅栏 II](https://leetcode.cn/problems/erect-the-fence-ii/)
- **难度**：困难
- **标签**：几何、随机化、递归、计算几何

## 1. 题目概述

给定二维平面上的点集 `trees`，其中 `trees[i] = [xi, yi]` 表示一棵树的位置。你需要用一个**圆形栅栏**把所有树围起来——找到能包围所有点的**最小圆**（半径最小的圆），返回圆心坐标和半径 `[x, y, r]`。
