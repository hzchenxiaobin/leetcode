...
- **题目名称**：找出每所学校的最低分数要求
- **链接**：[1988. Find Cutoff Score for Each School](https://leetcode.cn/problems/find-cutoff-score-for-each-school/)
- **难度**：中等
- **标签**：SQL、窗口函数、累计求和（SUM OVER）、关联子查询、COALESCE、CROSS JOIN、ROW_NUMBER、排序取 Top-1

## 1. 题目概述

给定 `Schools`（学校信息）与 `Exam`（考试成绩分布）两张表。每所学校有一个 `capacity`（最大招生人数）。`Exam` 表记录每个分数档的学生人数。学校设定一个**截止分数线**（cutoff score）：所有分数 ≥ cutoff 的学生被该校录取。
