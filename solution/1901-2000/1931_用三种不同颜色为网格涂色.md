...
- **题目名称**：用三种不同颜色为网格涂色
- **链接**：[1931. 用三种不同颜色为网格涂色](https://leetcode.cn/problems/painting-a-grid-with-three-different-colors/)
- **难度**：困难
- **标签**：动态规划、状态压缩、矩阵快速幂

## 1. 题目概述

给定两个整数 $m$ 和 $n$，构造一个 $m \times n$ 的网格，每个单元格初始为白色。用**红（R）、绿（G）、蓝（B）**三种颜色为每个单元格涂色，所有单元格都必须被涂色。
