...
- **题目名称**：反转单词前缀
- **链接**：[2000. 反转单词前缀](https://leetcode.cn/problems/reverse-prefix-of-word/)
- **难度**：简单
- **标签**：字符串、双指针

## 1. 题目概述

给你一个下标从 **0** 开始的字符串 `word` 和一个字符 `ch`。找出 `ch` **第一次出现**的下标 `i`，**反转** `word` 中从下标 `0` 开始、直到下标 `i` 结束（含下标 `i`）的那段字符。如果 `word` 中不存在字符 `ch`，则无需进行任何操作。返回 **结果字符串**。
