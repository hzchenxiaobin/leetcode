...
- **题目名称**：查询最大基因差
- **链接**：[1938. 查询最大基因差](https://leetcode.cn/problems/maximum-genetic-difference-query/)
- **难度**：困难
- **标签**：字典树、位运算、树、深度优先搜索、离线查询

## 1. 题目概述

给定一棵 $n$ 个节点的有根树，节点编号 $0$ 到 $n-1$，每个节点的**基因值**就是它的编号。两个基因值的**基因差**定义为两者的**异或和**（$\oplus$）。给你 `parents` 数组（`parents[i]` 为节点 $i$ 的父节点，根节点为 $-1$）和查询数组 `queries`，其中 `queries[i] = [node_i, val_i]`。
