...
- **题目名称**：统计重复个数
- **链接**：[466. 统计重复个数](https://leetcode.cn/problems/count-the-repetitions/)
- **难度**：困难
- **标签**：字符串、动态规划、循环节、子序列匹配

## 1. 题目概述

定义 `str = [s, n]` 表示 `str` 由 `n` 个字符串 `s` 顺序连接而成（即 `s` 重复 `n` 次）。若**从 `s2` 中删除若干字符**（不改变剩余字符顺序）能得到 `s1`，则称 `s1` 可由 `s2` 获得，等价于 **`s1` 是 `s2` 的子序列**。
