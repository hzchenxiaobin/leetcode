...
- **题目名称**：最优账单平衡
- **链接**：[465. 最优账单平衡](https://leetcode.cn/problems/optimal-account-balancing/)
- **难度**：困难
- **标签**：位运算、数组、动态规划、回溯、位掩码

## 1. 题目概述

给定一组朋友之间的借贷交易列表 `transactions`，其中 `transactions[i] = [from_i, to_i, amount_i]` 表示 `from_i` 给了 `to_i` 金额 `amount_i`。这些交易产生了人与人之间的净借贷关系。请返回**结清所有债务所需的最少交易笔数**。
