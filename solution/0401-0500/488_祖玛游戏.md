...
- **题目名称**：祖玛游戏
- **链接**：[488. 祖玛游戏](https://leetcode.cn/problems/zuma-game/)
- **难度**：困难
- **标签**：深度优先搜索、记忆化搜索、字符串、回溯

## 1. 题目概述

你在玩祖玛游戏的一个变体。棋盘上有一排球，手里也持有若干球。每次操作可以**从手中取一个球插入棋盘任意位置**；若插入后棋盘上出现 **3 个或以上连续同色球**，该组立即被消除；消除后两侧若因合并再次形成 3+ 连续同色，则**连锁消除**，直到没有可消段为止。
