...
- **题目名称**：最短独占单词缩写
- **链接**：[411. 最短独占单词缩写](https://leetcode.cn/problems/minimum-unique-word-abbreviation/)
- **难度**：困难
- **标签**：位运算、回溯、深度优先搜索、字符串

## 1. 题目概述

一个字符串 `word` 可以通过「缩写」来压缩：把其中**任意若干非空、不相邻**子串替换为它们的长度（数字串），其余字符保留原样；数字串不允许有前导零。例如 `"word"` 的全部合法缩写为：
