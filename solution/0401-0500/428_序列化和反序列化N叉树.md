...
- **题目名称**：序列化和反序列化 N 叉树
- **链接**：[428. 序列化和反序列化 N 叉树](https://leetcode.cn/problems/serialize-and-deserialize-n-ary-tree/)
- **难度**：困难
- **标签**：树、N 叉树、深度优先搜索、设计

## 1. 题目概述

设计一个算法来序列化与反序列化一棵 **N 叉树**。序列化是把一棵树转为**字符串**，反序列化是从该字符串**重建**出原来的树。不限制具体的序列化格式，但要求能无损往返。
