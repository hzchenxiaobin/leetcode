...
- **题目名称**：最小基因变化
- **链接**：[433. 最小基因变化](https://leetcode.cn/problems/minimum-genetic-mutation/)
- **难度**：中等
- **标签**：图、BFS、最短路、哈希

## 1. 题目概述

基因序列用长度为 `8` 的字符串表示，字符取自集合 `{'A', 'C', 'G', 'T'}`。一次**变异**指将序列中**某一个字符**替换为集合中的另一个字符，且变异后的序列必须出现在**基因库 `bank`** 中才算合法。
