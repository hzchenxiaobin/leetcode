...
给定一个区间数组 `intervals`，其中 `intervals[i] = [start_i, end_i]`，且每个 `start_i` **互不相同**。区间 `i` 的**右侧区间**是满足 `start_j >= end_i` 且 `start_j` **最小**的区间 `j`（注意 `i` 可能等于 `j`）。返回一个数组 `ans`，`ans[i]` 为区间 `i` 对应右侧区间的下标；若不存在则设为 `-1`。
