...
- **题目名称**：安排会议日程
- **链接**：[1229. 安排会议日程](https://leetcode.cn/problems/meeting-scheduler/)
- **难度**：中等
- **标签**：数组、双指针、区间、贪心

## 1. 题目概述

给定两个人的**可用时间段列表** `slots1` 和 `slots2`，以及一个会议时长 `duration`。每个时间段是一个闭区间 $[start, end]$，表示该人在 $[start, end]$ 内空闲。两个列表各自按起始时间**升序**排列且**内部互不重叠**。
