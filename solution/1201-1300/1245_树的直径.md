...
- **题目名称**：树的直径
- **链接**：[1245. 树的直径](https://leetcode.cn/problems/tree-diameter/)
- **难度**：中等
- **标签**：树、图、广度优先搜索、深度优先搜索

## 1. 题目概述

给你一棵**无向树**（连通无环图），以边列表 `edges` 表示，其中 `edges[i] = [u_i, v_i]` 表示节点 `u_i` 与 `v_i` 之间有一条无向边。返回这棵树的**直径**——即树中**任意两个节点之间最长路径**的**边数**。
