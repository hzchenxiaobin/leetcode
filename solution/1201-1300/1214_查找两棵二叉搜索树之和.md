...
- **题目名称**：查找两棵二叉搜索树之和
- **链接**：[1214. 查找两棵二叉搜索树之和](https://leetcode.cn/problems/two-sum-bsts/)
- **难度**：中等
- **标签**：树、二叉搜索树、哈希表、双指针、DFS

## 1. 题目概述

给定**两棵二叉搜索树（BST）** 的根节点 `root1` 和 `root2`，以及一个整数 `target`。判断是否存在 `root1` 中的一个节点 `a` 与 `root2` 中的一个节点 `b`，使得 $a.\text{val} + b.\text{val} = \text{target}$。存在返回 `true`，否则返回 `false`。
