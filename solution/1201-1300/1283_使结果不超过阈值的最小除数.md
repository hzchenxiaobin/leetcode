...
- **题目名称**：使结果不超过阈值的最小除数
- **链接**：[1283. 使结果不超过阈值的最小除数](https://leetcode.cn/problems/find-the-smallest-divisor-given-a-threshold/)
- **难度**：中等
- **标签**：数组、二分查找

## 1. 题目概述

给你一个整数数组 `nums` 和一个正整数 `threshold`，你需要选择一个正整数作为**除数**，然后将数组里每个数都除以它，并对除法结果**求和**（每个数除以除数后都**向上取整**）。
