...
- **题目名称**：网格中的最短路径
- **链接**：[1293. 网格中的最短路径](https://leetcode.cn/problems/shortest-path-in-a-grid-with-obstacles-elimination/)
- **难度**：困难
- **标签**：广度优先搜索、矩阵、最短路径、状态压缩

## 1. 题目概述

给定一个 `m × n` 的网格 `grid`，每个格子为 `0`（空）或 `1`（障碍）。从左上角 `(0,0)` 出发走到右下角 `(m-1, n-1)`，每步可走到上下左右相邻格子。你**最多可以消除 `k` 个障碍**（把经过的 `1` 当成 `0` 走过，每个障碍只能消除一次）。求到达终点的**最少步数**；若无法到达返回 `-1`。
