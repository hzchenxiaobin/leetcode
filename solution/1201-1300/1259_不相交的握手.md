...
- **题目名称**：不相交的握手
- **链接**：[1259. 不相交的握手](https://leetcode.cn/problems/handshakes-that-dont-cross/)
- **难度**：困难
- **标签**：动态规划、数学、卡塔兰数

## 1. 题目概述

`numPeople` 个人围成一圈，编号为 `0` 到 `numPeople - 1`。每个人与另一个人握手，且**任意两次握手之间不能交叉**（即在圆环内画弦，弦与弦不能相交）。返回满足条件的握手方案数，对 $10^9 + 7$ 取模。
