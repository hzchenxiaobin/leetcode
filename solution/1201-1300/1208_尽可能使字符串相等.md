...
- **题目名称**：尽可能使字符串相等
- **链接**：[1208. 尽可能使字符串相等](https://leetcode.cn/problems/get-equal-substrings-within-budget/)
- **难度**：中等
- **标签**：字符串、滑动窗口、数组

## 1. 题目概述

给你两个**长度相同**的字符串 `s` 和 `t`。将 `s` 的第 `i` 个字符变到 `t` 的第 `i` 个字符需要 $|s[i] - t[i]|$ 的开销（两个字符 ASCII 码值差的绝对值，可能为 0）。
