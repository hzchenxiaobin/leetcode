...
- **题目名称**：转变数组后最接近目标值的数组和
- **链接**：[1300. 转变数组后最接近目标值的数组和](https://leetcode.cn/problems/sum-of-mutated-array-closest-to-target/)
- **难度**：中等
- **标签**：数组、二分查找、排序、前缀和

## 1. 题目概述

给你一个整数数组 `arr` 和一个目标值 `target`。请你从数组中选出一个整数 `value`，将数组中所有比 `value` 大的元素变成 `value`（即对每个元素取 $\min(\text{arr}[i], \text{value})$），使得转变后数组的元素和**最接近** `target`。
