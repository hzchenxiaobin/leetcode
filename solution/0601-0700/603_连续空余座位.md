...
- **题目名称**：连续空余座位
- **链接**：[603. 连续空余座位](https://leetcode.cn/problems/consecutive-available-seats/)
- **难度**：简单
- **标签**：SQL、自连接、窗口函数、LAG、LEAD、连续性问题

## 1. 题目概述

给定 `Cinema` 表，每行记录一个座位的占用情况：座位号 `seat_id` 与空闲标志 `free`（`1` 表示空余，`0` 表示被占）。编写 SQL 查询，找出**所有连续空余的座位号**，按 `seat_id` 升序返回。
