...
- **题目名称**：有趣的电影
- **链接**：[620. 有趣的电影](https://leetcode.cn/problems/not-boring-movies/)
- **难度**：简单
- **标签**：SQL、WHERE、ORDER BY、MOD、位运算

## 1. 题目概述

给定 `cinema` 表，每行记录一部电影的 `id`、`movie`（名称）、`description`（描述）、`rating`（评分）。编写 SQL 查询，找出所有**描述为非 `boring`** 且 **`id` 为奇数**的影片，并按 `rating` **降序**排列。
