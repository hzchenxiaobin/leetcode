...
- **题目名称**：只出现一次的最大数字
- **链接**：[619. 只出现一次的最大数字](https://leetcode.cn/problems/biggest-single-number/)
- **难度**：简单
- **标签**：SQL、GROUP BY、HAVING、COUNT、MAX、聚合、子查询

## 1. 题目概述

`MyNumbers` 表只含一列 `num`（整数），**无主键**，因此可能包含重复值。**单一数字**是在表中只出现一次的数字。要求找出**最大的单一数字**；如果不存在单一数字，则返回 `null`。
