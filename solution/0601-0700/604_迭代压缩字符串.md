...
- **题目名称**：迭代压缩字符串
- **链接**：[604. 迭代压缩字符串](https://leetcode.cn/problems/design-compressed-string-iterator/)
- **难度**：简单
- **标签**：设计、字符串、迭代器、指针

## 1. 题目概述

设计一个**压缩字符串迭代器**。压缩串的形式为「字母 + 正整数」交替拼接，例如 `L1e2t1C1o1d1e1` 表示字母 `L` 出现 1 次、`e` 出现 2 次、`t` 出现 1 次……展开后即 `LeetCode`。要求实现：
