...
- **题目名称**：销售员
- **链接**：[607. 销售员](https://leetcode.cn/problems/sales-person/)
- **难度**：简单
- **标签**：SQL、反连接、NOT IN、LEFT JOIN、NOT EXISTS、多表 JOIN、子查询

## 1. 题目概述

给定三张表——`SalesPerson`（销售员）、`Company`（公司）、`Orders`（订单），编写 SQL 查询找出**没有任何与名为 "RED" 的公司相关订单的所有销售人员姓名**。结果只返回 `name` 列，顺序任意。
