...
- **题目名称**：体育馆的人流量
- **链接**：[601. 体育馆的人流量](https://leetcode.cn/problems/human-traffic-of-stadium/)
- **难度**：困难
- **标签**：SQL、自连接、窗口函数、LAG、LEAD、岛屿问题

## 1. 题目概述

给定 `Stadium` 表，每行记录某天体育馆的人流量。编写 SQL 查询，找出**人数大于等于 100** 且**连续 id 不少于 3 行**的所有记录，结果按 `visit_date` 升序返回**完整行**（`id`、`visit_date`、`people` 三列）。
