...
- **题目名称**：设计日志存储系统
- **链接**：[635. 设计日志存储系统](https://leetcode.cn/problems/design-log-storage-system/)
- **难度**：中等
- **标签**：设计、哈希表、字符串

## 1. 题目概述

设计一个日志存储系统，每条日志包含一个唯一 `id` 和一个时间戳 `timestamp`。时间戳格式固定为 `Year:Month:Day:Hour:Minute:Second`，例如 `"2017:01:01:23:59:59"`，**所有字段均为零填充的十进制数**。
