...
- **题目名称**：大礼包
- **链接**：[638. 大礼包](https://leetcode.cn/problems/shopping-offers/)
- **难度**：中等
- **标签**：动态规划、记忆化搜索、数组

## 1. 题目概述

在 LeetCode 商店中有 `n` 件在售物品，每件物品都有单价 `price[i]`。同时有一些**大礼包**，每个大礼包以优惠价捆绑销售一组物品。`special[i]` 长度为 `n + 1`，前 `n` 个数表示礼包内含各物品数量，最后一个数为礼包价格。
