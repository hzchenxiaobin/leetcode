...
- **题目名称**：设计循环双端队列
- **链接**：[641. 设计循环双端队列](https://leetcode.cn/problems/design-circular-deque/)
- **难度**：中等
- **标签**：设计、队列、数组、双端队列

## 1. 题目概述

设计一个**循环双端队列**（circular deque）：在 [622 循环队列](../0601-0700/622_设计循环队列.md)「环形数组 + 空间复用」的基础上，**两端都能插入和删除**。即队首（front）和队尾（rear）各自支持入队/出队，共 4 个写操作。
