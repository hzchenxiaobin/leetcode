...
- **题目名称**：平面上的最近距离
- **链接**：[612. 平面上的最近距离](https://leetcode.cn/problems/shortest-distance-in-a-plane/)
- **难度**：中等
- **标签**：SQL、自连接、聚合、MIN、数学、窗口函数

## 1. 题目概述

表 `point_2d` 存储了平面上若干**互不相同**的点（多于 2 个）的坐标。编写 SQL 查询，找出**任意两点之间的最短距离**，结果四舍五入到小数点后 2 位，输出列名为 `shortest`。
