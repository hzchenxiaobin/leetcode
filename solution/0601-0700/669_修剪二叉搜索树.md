...
给你**二叉搜索树（BST）**的根节点 `root`，同时给定最小边界 `low` 和最大边界 `high`。通过修剪 BST，使得**所有节点的值落在 $[\text{low}, \text{high}]$ 中**。修剪**不应改变**保留在树中的元素的相对结构（即未被移除的节点之间原有的父子关系应当保留）。可以证明存在**唯一答案**。返回修剪后的 BST 新的根节点。
