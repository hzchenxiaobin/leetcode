...
- **题目名称**：二叉树最大宽度
- **链接**：[662. 二叉树最大宽度](https://leetcode.cn/problems/maximum-width-of-binary-tree/)
- **难度**：中等
- **标签**：树、二叉树、广度优先搜索（BFS）

## 1. 题目概述

给定一棵二叉树的根节点 `root`，返回树的**最大宽度**。某一层的**宽度**定义为该层**最左**和**最右**两个非空节点（即两端节点）之间的长度，**两端节点之间的 `null` 节点也计入长度**。
