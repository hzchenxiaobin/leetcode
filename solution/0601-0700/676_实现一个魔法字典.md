...
- **题目名称**：实现一个魔法字典
- **链接**：[676. 实现一个魔法字典](https://leetcode.cn/problems/implement-magic-dictionary/)
- **难度**：中等
- **标签**：字典树、深度优先搜索、哈希表、字符串、设计

## 1. 题目概述

设计一个 **MagicDictionary**：先调用 `buildDict(dictionary)` 把一批**互不相同**的单词存进去；之后 `search(searchWord)` 判断——能否把 `searchWord` **恰好一个字符**替换成另一个小写字母，使所得字符串命中字典里的某个单词。
