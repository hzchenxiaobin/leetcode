...
- **题目名称**：变更性别
- **链接**：[627. 变更性别](https://leetcode.cn/problems/swap-salary/)
- **难度**：简单
- **标签**：SQL、UPDATE、CASE WHEN、IF、位运算、ASCII

## 1. 题目概述

给定 `salary` 表，每行存放一个员工的 `id`、`name`、`sex`（取值 `'m'` 或 `'f'`）、`salary`。编写**单条 UPDATE 语句**，将所有 `'m'` 改成 `'f'`、`'f'` 改成 `'m'`（即翻转 `sex` 列），不使用任何临时表或中间变量。
