...
- **题目名称**：感染二叉树需要的总时间
- **链接**：[2385. 感染二叉树需要的总时间](https://leetcode.cn/problems/amount-of-time-for-binary-tree-to-be-infected/)
- **难度**：中等
- **标签**：树、深度优先搜索、广度优先搜索、二叉树

## 1. 题目概述

给你一棵二叉树的根 `root`，节点值**互不相同**；另给整数 `start`。第 `0` 分钟，值为 `start` 的节点被感染。每分钟，**任一与已感染节点相邻且尚未感染的节点**都会被感染（相邻 = 父子边相连，**双向可达**）。返回感染整棵树所需的分钟数。
