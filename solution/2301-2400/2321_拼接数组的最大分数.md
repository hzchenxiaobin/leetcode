...
给你两个下标从 **0** 开始的整数数组 `nums1` 和 `nums2`，长度都是 `n`。你可以选择两个整数 `left` 和 `right`（$0 \le \text{left} \le \text{right} \lt n$），**交换**两个子数组 `nums1[left...right]` 与 `nums2[left...right]`（可执行一次或不执行）。交换后，数组的**分数**取 $\max(\sum \text{nums1},\ \sum \text{nums2})$。返回可能的最大分数。
