...
- **题目名称**：产品销售分析 V
- **链接**：[2329. 产品销售分析 V 🔒](https://leetcode.cn/problems/product-sales-analysis-v/)
- **难度**：简单
- **标签**：数据库、SQL、`JOIN`、`USING`、`GROUP BY`、`SUM`、聚合派生度量、`ORDER BY` 多列排序、并列 tie-break、LeetCode 锁题

## 1. 题目概述

给定 `Sales` 表（销售记录，含购买者 `user_id` 与数量 `quantity`）与 `Product` 表（产品单价 `price`），编写 SQL 查询，返回**每个用户的消费额** `spending`，其中某用户的消费额为该用户所有销售金额（$\text{quantity} \times \text{price}$）之和。结果按 `spending` **递减**排序；**消费额相等时**按 `user_id` **递增**排序。
