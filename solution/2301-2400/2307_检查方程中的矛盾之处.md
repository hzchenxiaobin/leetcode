...
- **题目名称**：检查方程中的矛盾之处
- **链接**：[2307. 检查方程中的矛盾之处](https://leetcode.cn/problems/check-for-contradictions-in-equations/)
- **难度**：困难
- **标签**：深度优先搜索、并查集、图、数组、字符串

## 1. 题目概述

给定一个字符串二维数组 `equations` 和一个实数数组 `values`，其中 `equations[i] = [Ai, Bi]` 且 `values[i]` 表示 `Ai / Bi = values[i]`。判断这些等式是否存在**矛盾**：存在矛盾返回 `true`，否则返回 `false`。
