...
- **题目名称**：删除操作后的最大子段和
- **链接**：[2382. 删除操作后的最大子段和](https://leetcode.cn/problems/maximum-segment-sum-after-removals/)
- **难度**：困难
- **标签**：并查集、数组、逆序处理

## 1. 题目概述

给你两个下标从 **0** 开始的整数数组 `nums` 和 `removeQueries`，长度都为 `n`。第 `i` 次操作把 `nums` 中下标 `removeQueries[i]` 处的元素删除（视作置 `0`），将 `nums` 分割成更小的子段。**子段**是 `nums` 中连续的、未被删除元素形成的序列，**子段和**是子段中所有元素之和。请返回长度为 `n` 的数组 `answer`，其中 `answer[i]` 是第 `i` 次删除操作后的**最大**子段和。每个下标至多删除一次。
