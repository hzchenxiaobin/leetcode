...
- **题目名称**：受限条件下可到达节点的数目
- **链接**：[2368. 受限条件下可到达节点的数目](https://leetcode.cn/problems/reachable-nodes-with-restrictions/)
- **难度**：中等
- **标签**：树、深度优先搜索、广度优先搜索、图、哈希表

## 1. 题目概述

现有一棵由 `n` 个节点组成的**无向树**，节点编号从 `0` 到 `n - 1`，共有 `n - 1` 条边。给你一个二维整数数组 `edges`（长度为 `n - 1`），其中 `edges[i] = [ai, bi]` 表示节点 `ai` 和 `bi` 之间存在一条边。另给你一个整数数组 `restricted` 表示**受限**节点。
