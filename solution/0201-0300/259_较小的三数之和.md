...
- **题目名称**：较小的三数之和
- **链接**：[259. 较小的三数之和](https://leetcode.cn/problems/3sum-smaller/)
- **难度**：中等
- **标签**：数组、双指针、排序、二分查找

## 1. 题目概述

给定一个长度为 $n$ 的整数数组 `nums` 和一个目标值 `target`，返回满足 **$i < j < k$** 且 $\text{nums}[i] + \text{nums}[j] + \text{nums}[k] < \text{target}$ 的下标三元组 `(i, j, k)` 的**个数**。
