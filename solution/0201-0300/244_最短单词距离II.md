...
- **题目名称**：最短单词距离II
- **链接**：[244. 最短单词距离 II](https://leetcode.cn/problems/shortest-word-distance-ii/)
- **难度**：中等
- **标签**：设计、数组、哈希表、双指针

## 1. 题目概述

请设计一个类，接收一个字符串数组 `wordsDict`，并支持**多次**查询：给定两个**不同**的单词 `word1`、`word2`（二者都保证出现在数组中），返回它们在数组中的**最短距离**——即各自出现位置下标之差的绝对值的最小值 $|i - j|$。
