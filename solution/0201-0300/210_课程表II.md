...
- **题目名称**：课程表 II
- **链接**：[210. 课程表 II](https://leetcode.cn/problems/course-schedule-ii/)
- **难度**：中等
- **标签**：深度优先搜索、广度优先搜索、图、拓扑排序

## 1. 题目概述

你这个学期必须选修 `numCourses` 门课程，记为 `0` 到 `numCourses - 1`。数组 `prerequisites` 中 `prerequisites[i] = [a, b]` 表示**想学 `a` 必须先学 `b`**。请你返回**一个可行的选课顺序**（拓扑序）；若无法完成所有课程（即存在环），返回**空数组**。
