...
- **题目名称**：最短单词距离
- **链接**：[243. 最短单词距离](https://leetcode.cn/problems/shortest-word-distance/)
- **难度**：中等
- **标签**：数组、字符串、双指针、单遍扫描

## 1. 题目概述

给定一个字符串数组 `wordsDict` 和两个**不同**的字符串 `word1`、`word2`（二者都保证出现在数组中），返回这两个单词在数组中的**最短距离**——即各自出现位置下标之差的绝对值的最小值 $|i - j|$。
