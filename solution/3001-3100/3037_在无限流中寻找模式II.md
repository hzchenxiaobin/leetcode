...
- **题目名称**：在无限流中寻找模式 II
- **链接**：[3037. 在无限流中寻找模式 II](https://leetcode.cn/problems/find-pattern-in-infinite-stream-ii/)
- **难度**：困难
...
给定一个二进制数组 `pattern`（只含 0/1）和一个 `InfiniteStream` 类的对象 `stream`，表示一条**下标从 0 开始的二进制位无限流**。
