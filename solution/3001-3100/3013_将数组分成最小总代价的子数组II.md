...
- **题目名称**：将数组分成最小总代价的子数组 II
- **链接**：[3013. 将数组分成最小总代价的子数组 II](https://leetcode.cn/problems/divide-an-array-into-subarrays-with-minimum-cost-ii/)
- **难度**：困难
- **标签**：数组、滑动窗口、有序集合、双多重集、堆

## 1. 题目概述

给定下标从 0 开始、长度为 $n$ 的整数数组 `nums` 与两个**正**整数 `k` 和 `dist`。一个数组的**代价**是其中的**第一个**元素（如 `[1,2,3]` 代价为 `1`，`[3,4,1]` 代价为 `3`）。
