...
- **题目名称**：找出网格的区域平均强度
- **链接**：[3030. 找出网格的区域平均强度](https://leetcode.cn/problems/find-the-grid-of-region-average/)
- **难度**：中等
- **标签**：数组、矩阵、枚举

## 1. 题目概述

给你一个下标从 **0** 开始、大小为 $m \times n$ 的网格 `image`，表示一张灰度图，其中 `image[i][j]` 是 $[0..255]$ 内的像素强度。另给一个**非负**整数 `threshold`。
