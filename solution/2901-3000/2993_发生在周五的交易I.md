...
- **题目名称**：发生在周五的交易 I
- **链接**：[2993. 发生在周五的交易 I](https://leetcode.cn/problems/friday-purchases-i/)
- **难度**：中等
...
给定一张交易表 `Purchases`，每行记录某用户在某天的消费金额。编写解决方案，统计 **2023 年 11 月每个周五的全平台总消费金额**：把同属一个周五的多笔消费求和，并标记它属于当月的第几个周五（`week_of_month`）。**若某个周五没有任何交易，该周不进入结果表**。按 `week_of_month` 升序返回。
