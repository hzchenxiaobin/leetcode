...
- **题目名称**：寻找房价最贵的城市
- **链接**：[2987. 寻找房价最贵的城市](https://leetcode.cn/problems/find-expensive-cities/)
- **难度**：简单
...
给定房源挂牌表 `Listings`，每行是一套待售房源：所在城市 `city` 与挂牌价 `price`。编写解决方案，找出**平均房价**超过**全国**（即全部房源整体）**平均房价**的**城市**，返回按 `city` **升序**排序的结果表，输出单列 `city`。
