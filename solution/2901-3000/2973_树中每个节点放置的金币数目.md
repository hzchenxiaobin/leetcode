...
- **题目名称**：树中每个节点放置的金币数目
- **链接**：[2973. 树中每个节点放置的金币数目](https://leetcode.cn/problems/find-number-of-coins-to-place-in-tree-nodes/)
- **难度**：困难
- **标签**：树、深度优先搜索、树形 DP、排序

## 1. 题目概述

给你一棵 $n$ 个节点的**无向**树，节点编号 $0$ 到 $n-1$，根节点为 $0$；`edges[i] = [ai, bi]` 表示节点 $a_i$ 与 $b_i$ 之间有一条边。再给长度为 $n$ 的整数数组 `cost`，`cost[i]` 是第 $i$ 个节点的**开销**（可正可负但非零）。
