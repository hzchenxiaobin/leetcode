...
- **题目名称**：观众变主播
- **链接**：[2995. 观众变主播](https://leetcode.cn/problems/viewers-turned-streamers/)
- **难度**：困难
...
直播平台记录每个用户的全部会话：每个会话要么是**观众场**（`Viewer`，看直播），要么是**主播场**（`Streamer`，开直播）。编写解决方案，找出**首次会话（按 `session_start` 最早）是观众身份**的用户，统计他们各自的**主播会话数量**，按「会话数降序、`user_id` 降序」返回结果表。
