...
- **题目名称**：最小化舍入误差以满足目标
- **链接**：[1058. 最小化舍入误差以满足目标](https://leetcode.cn/problems/minimize-rounding-error-to-meet-target/)
- **难度**：中等
- **标签**：贪心、数组、字符串、数学

## 1. 题目概述

给定一组用字符串表示的价格 `prices` 和一个整数目标 `target`。每个价格必须被**向下取整（floor）或向上取整（ceil）**为整数。所有取整后的价格之和必须**恰好等于** `target`。
