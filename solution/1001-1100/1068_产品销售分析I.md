...
- **题目名称**：产品销售分析 I
- **链接**：[1068. 产品销售分析 I](https://leetcode.cn/problems/product-sales-analysis-i/)
- **难度**：简单
- **标签**：SQL、数据库、JOIN、INNER JOIN、多表关联

## 1. 题目概述

给定 `Sales` 表（销售记录）与 `Product` 表（产品信息），编写 SQL 查询，返回 `Sales` 表中所有销售记录对应的 `product_name` 以及该产品的 `year` 和 `price`。结果无顺序要求。
