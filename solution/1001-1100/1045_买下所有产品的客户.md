...
- **题目名称**：买下所有产品的客户
- **链接**：[1045. 买下所有产品的客户](https://leetcode.cn/problems/customers-who-bought-all-products/)
- **难度**：中等
- **标签**：SQL、GROUP BY、HAVING、COUNT(DISTINCT)、关系除法、NOT EXISTS、子查询

## 1. 题目概述

给定 `Customer` 表（客户购买记录）与 `Product` 表（产品全集），编写 SQL 查询报告 **购买了 `Product` 表中所有产品** 的客户的 `id`。结果无顺序要求。
