...
- **题目名称**：从起点到终点的所有路径
- **链接**：[1059. 从起点到终点的所有路径](https://leetcode.cn/problems/all-paths-from-source-lead-to-destination/)
- **难度**：中等
- **标签**：深度优先搜索、图、三色标记法

## 1. 题目概述

给定一个有向图的边集合 `edges`，以及图中的起点 `source` 和终点 `destination`。判断**从 `source` 出发的所有路径是否最终都终止于 `destination`**，当且仅当满足以下三个条件时返回 `true`：
