...
- **题目名称**：前五科的均分
- **链接**：[1086. High Five](https://leetcode.cn/problems/high-five/)
- **难度**：简单
- **标签**：数组、哈希表、排序、堆（优先队列）
...
给定一个二维数组 `items`，其中 `items[i] = [student_id, score]`，表示某学生在某科目获得的成绩。每个学生**至少有 5 条成绩记录**。请为每个学生计算其**最高的 5 个成绩的整数平均值**，返回一个按 `student_id` **升序排列**的二维数组，每个元素为 `[student_id, average]`。
