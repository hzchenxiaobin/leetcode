...
- **题目名称**：元素和为目标值的子矩阵数量
- **链接**：[1074. 元素和为目标值的子矩阵数量](https://leetcode.cn/problems/number-of-submatrices-that-sum-to-target/)
- **难度**：困难
- **标签**：数组、哈希表、矩阵、前缀和

## 1. 题目概述

给定一个 `m × n` 的整数矩阵 `matrix` 和一个整数 `target`，返回元素之和等于 `target` 的**非空子矩阵的数量**。
