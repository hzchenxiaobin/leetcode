...
- **题目名称**：两个无重叠子数组的最大和
- **链接**：[1031. 两个无重叠子数组的最大和](https://leetcode.cn/problems/maximum-sum-of-two-non-overlapping-subarrays/)
- **难度**：中等
- **标签**：数组、动态规划、前缀和

## 1. 题目概述

给定一个整数数组 `nums` 和两个整数 `firstLen` 与 `secondLen`，找出两个**无重叠**的连续子数组，长度分别为 `firstLen` 和 `secondLen`，使二者元素之和最大，返回该最大和。
