...
- **题目名称**：产品销售分析 III
- **链接**：[1070. 产品销售分析 III](https://leetcode.cn/problems/product-sales-analysis-iii/)
- **难度**：中等
- **标签**：SQL、GROUP BY、MIN、子查询、JOIN、窗口函数、RANK

## 1. 题目概述

给定销售表 `Sales`，每一行表示编号 `product_id` 的产品在某一年的销售额。编写 SQL 查询，选出**每个售出过的产品第一年**（即首次出现的最早年份）销售的 `product_id`、`年份`、`数量` 和 `价格`。
