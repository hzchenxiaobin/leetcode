...
- **题目名称**：按字典序排列最小的等效字符串
- **链接**：[1061. 按字典序排列最小的等效字符串](https://leetcode.cn/problems/lexicographically-smallest-equivalent-string/)
- **难度**：中等
- **标签**：并查集、字符串

## 1. 题目概述

给出长度相同的两个字符串 `s1` 和 `s2`，还有一个字符串 `baseStr`。其中 `s1[i]` 和 `s2[i]` 是一组等价字符。
