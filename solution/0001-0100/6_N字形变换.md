...
- **题目名称**：N 字形变换
- **链接**：[6. N 字形变换](https://leetcode.cn/problems/zigzag-conversion/)
- **难度**：中等
- **标签**：字符串

## 1. 题目概述

给定一个字符串 `s` 和一个整数 `numRows`，把 `s` 按「N 字形」（即先竖直向下、再斜向上）排列成 `numRows` 行，最后**按行从左到右、从上到下**依次读出，得到新字符串。
...
- [6. N 字形变换](https://leetcode.cn/problems/zigzag-conversion/)（[题解](N字形变换.md)）：周期规律 + 按行收集
...
- [54. 螺旋矩阵](https://leetcode.cn/problems/spiral-matrix/)（[题解](../../week1/day2/螺旋矩阵.md)）：按方向遍历矩阵，方向切换与边界处理同理
