...
- **题目名称**：删除有序数组中的重复项 II
- **链接**：[80. 删除有序数组中的重复项 II](https://leetcode.cn/problems/remove-duplicates-from-sorted-array-ii/)
- **难度**：中等
- **标签**：数组、双指针

## 1. 题目概述

给定一个 **已按非递减顺序排列** 的整数数组 `nums`，请你**原地**删除重复元素，使每个元素最多出现两次。最终结果的前 `k` 个位置应存放去重后的元素，`k` 之后的元素忽略。返回 `k`。
