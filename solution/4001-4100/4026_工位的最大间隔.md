...
- **题目名称**：工位的最大间隔
- **链接**：[4026. 工位的最大间隔](https://leetcode.cn/problems/maximum-gap-between-stations/)
- **难度**：中等
- **标签**：贪心、双指针、字符串

## 1. 题目概述

给你两个长度分别为 $n$ 和 $m$ 的字符串 `skill` 和 `station`：`skill[i]` 表示工人 $i$ 的技能，`station[j]` 表示工位 $j$ 所支持的技能。
