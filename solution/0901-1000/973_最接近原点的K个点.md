...
- **题目名称**：最接近原点的 K 个点
- **链接**：[973. 最接近原点的 K 个点](https://leetcode.cn/problems/k-closest-points-to-origin/)
- **难度**：中等
- **标签**：数组、分治、快速选择、堆

## 1. 题目概述

给定一个数组 `points`，其中 `points[i] = [xi, yi]` 表示 X-Y 平面上的一个点，以及一个整数 `k`。返回**离原点 `(0, 0)` 最近的 `k` 个点**。距离按**欧几里得距离**计算。答案可以按**任意顺序**返回。
