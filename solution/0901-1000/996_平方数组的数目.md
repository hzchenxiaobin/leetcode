...
- **题目名称**：平方数组的数目
- **链接**：[996. 平方数组的数目](https://leetcode.cn/problems/number-of-squareful-arrays/)
- **难度**：困难
- **标签**：回溯、排列、排序、数学

## 1. 题目概述

如果一个数组的**任意两个相邻元素之和都是完全平方数**，则该数组称为**平方数组**。给定一个整数数组 `nums`，返回所有属于平方数组的 `nums` 的**排列数量**。如果存在某个索引 `i` 使得 `perm1[i] != perm2[i]`，则认为两个排列不同。
