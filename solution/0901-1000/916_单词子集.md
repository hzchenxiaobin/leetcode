...
- **题目名称**：单词子集
- **链接**：[916. 单词子集](https://leetcode.cn/problems/word-subsets/)
- **难度**：中等
- **标签**：数组、哈希表、字符串、计数

## 1. 题目概述

给你两个字符串数组 `words1` 和 `words2`。如果字符串 `b` 中的每个字母都出现在 `a` 中（**包括重复出现的字母**），则称 `b` 是 `a` 的**子集**。例如 `"wrr"` 是 `"warrior"` 的子集，但不是 `"world"` 的子集。
