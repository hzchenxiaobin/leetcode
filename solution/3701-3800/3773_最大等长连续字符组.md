...
- **题目名称**：最大等长连续字符组
- **链接**：[3773. 最大等长连续字符组](https://leetcode.cn/problems/maximum-number-of-equal-length-runs/)
- **难度**：中等
- **标签**：哈希表、字符串、计数
...
给定一个字符串 `s`。把 `s` 切分成若干**连续字符组**（run）：每段由**连续相同的字符**组成，且不能再向两边延长。切分方式天然唯一——在每处「相邻字符不同」的位置切开即可。
