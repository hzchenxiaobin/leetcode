...
- **题目名称**：统计稳定子数组的数目
- **链接**：[3748. 统计稳定子数组的数目](https://leetcode.cn/problems/count-stable-subarrays/)
- **难度**：困难
- **标签**：数组、二分查找、前缀和

## 1. 题目概述

给你一个整数数组 `nums`。如果 `nums` 的一个**子数组**中**没有逆序对**——即不存在满足 $i < j$ 且 $\text{nums}[i] > \text{nums}[j]$ 的下标对——则该子数组被称为**稳定**子数组。
