...
- **题目名称**：路径交叉
- **链接**：[335. 路径交叉](https://leetcode.cn/problems/self-crossing/)
- **难度**：困难
- **标签**：几何、数组、数学

## 1. 题目概述

从 X-Y 平面上的点 $(0,0)$ 开始，先向北移动 `distance[0]` 米，然后向西移动 `distance[1]` 米，向南移动 `distance[2]` 米，向东移动 `distance[3]` 米，持续移动。每次移动后方位**逆时针变化**（N→W→S→E→N→…）。判断所经过的路径是否**自交**（相交返回 `true`，否则返回 `false`）。
