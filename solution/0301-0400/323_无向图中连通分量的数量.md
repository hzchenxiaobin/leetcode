...
- **题目名称**：无向图中连通分量的数量
- **链接**：[323. 无向图中连通分量的数量](https://leetcode.cn/problems/number-of-connected-components-in-an-undirected-graph/)
- **难度**：中等
- **标签**：并查集、深度优先搜索、广度优先搜索、图

## 1. 题目概述

给定一个由 `n` 个节点（编号 `0` 到 `n - 1`）组成的无向图，以及一个边列表 `edges`，其中 `edges[i] = [ai, bi]` 表示节点 `ai` 与 `bi` 之间存在一条无向边。
