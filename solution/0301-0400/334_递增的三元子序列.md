...
- **题目名称**：递增的三元子序列
- **链接**：[334. 递增的三元子序列](https://leetcode.cn/problems/increasing-triplet-subsequence/)
- **难度**：中等
- **标签**：数组、贪心

## 1. 题目概述

给你一个整数数组 `nums`，判断是否存在三元组 `(i, j, k)` 满足 `i < j < k` 且 `nums[i] < nums[j] < nums[k]`。若存在返回 `true`，否则返回 `false`。
...
- [392. 判断子序列](https://leetcode.cn/problems/is-subsequence/)：双指针判定子序列，另一种「保序匹配」思路
