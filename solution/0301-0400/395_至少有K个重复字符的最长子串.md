...
- **题目名称**：至少有 K 个重复字符的最长子串
- **链接**：[395. 至少有 K 个重复字符的最长子串](https://leetcode.cn/problems/longest-substring-with-at-least-k-repeating-characters/)
- **难度**：中等
- **标签**：字符串、分治、滑动窗口、哈希表

## 1. 题目概述

给定一个字符串 `s` 和一个整数 `k`，找出 `s` 中**最长子串**的长度，要求该子串中**每个出现的字符都至少出现 `k` 次**。
