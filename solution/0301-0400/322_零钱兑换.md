...
> 💡 与 [Week8 Day3 面试基础篇](../../../ai-infra-notes/aiinfra/daily/week8/day3/README.md) 的 GPU **shared memory tile 复用**同构——DP 中 `dp[i]` 查 `dp[i-coin]`（已算好的子问题解，不重算），GPU kernel 从 shared memory 读 tile（已从 HBM 加载的数据，不重读）。两者都是"缓存已计算/已加载的结果，避免重复"的核心模式。DP 表 = shared memory tile = 缓存。
