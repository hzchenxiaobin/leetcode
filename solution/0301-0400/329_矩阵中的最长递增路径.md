...
- **题目名称**：矩阵中的最长递增路径
- **链接**：[329. 矩阵中的最长递增路径](https://leetcode.cn/problems/longest-increasing-path-in-a-matrix/)
- **难度**：困难
- **标签**：深度优先搜索、记忆化搜索、动态规划、拓扑排序、图

## 1. 题目概述

给定一个 `m x n` 的整数矩阵 `matrix`，找出其中**最长严格递增路径**的长度。从每个格子出发，可以向上、下、左、右四个方向移动，但只能移动到**值严格更大**的相邻格子。可以任意选择起点，路径可以开始于任意格子。
...
- [207. 课程表](https://leetcode.cn/problems/course-schedule/)（[题解](../0101-0200/207_课程表.md)）：拓扑排序模板题，本题扩展解法的 Kahn BFS 即源于此
