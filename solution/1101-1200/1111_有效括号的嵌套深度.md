...
- **题目名称**：有效括号的嵌套深度
- **链接**：[1111. 有效括号的嵌套深度](https://leetcode.cn/problems/maximum-nesting-depth-of-two-valid-parentheses-strings/)
- **难度**：中等
- **标签**：栈、贪心、字符串

## 1. 题目概述

给定一个有效括号字符串（VPS）`seq`，将其拆分成两个**不相交的子序列** `A` 和 `B`（`A` 和 `B` 都必须是 VPS，且 `A.length + B.length = seq.length`）。子序列不需要连续。
