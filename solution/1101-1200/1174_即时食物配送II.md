...
- **题目名称**：即时食物配送 II
- **链接**：[1174. 即时食物配送 II](https://leetcode.cn/problems/immediate-food-delivery-ii/)
- **难度**：中等
- **标签**：SQL、子查询、GROUP BY、MIN 聚合、JOIN、AVG、布尔转比例

## 1. 题目概述

给定 `Delivery` 表，每行记录一次食物配送：某顾客在某日下单，并指定了一个期望配送日期（≥ 下单日）。如果期望配送日期与下单日期相同，则称为「即时订单」，否则称为「计划订单」。「首次订单」是每位顾客最早创建的订单（题目保证每位顾客只有一个首次订单）。编写 SQL 查询，计算**即时订单在所有顾客首次订单中的比例**，保留两位小数。
