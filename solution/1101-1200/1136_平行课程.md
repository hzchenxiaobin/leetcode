...
- **题目名称**：平行课程
- **链接**：[1136. 平行课程](https://leetcode.cn/problems/parallel-courses/)
- **难度**：中等
- **标签**：图、广度优先搜索、拓扑排序、动态规划

## 1. 题目概述

给定 `n` 门课程，编号 `1` 到 `n`，以及数组 `relations`，其中 `relations[i] = [prevCoursei, nextCoursei]` 表示**必须先修 `prevCoursei` 才能修 `nextCoursei`**。
