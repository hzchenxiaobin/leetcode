...
- **题目名称**：文章浏览 I
- **链接**：[1148. 文章浏览 I](https://leetcode.cn/problems/article-views-i/)
- **难度**：简单
- **标签**：SQL、WHERE 过滤、DISTINCT、去重、ORDER BY、列别名

## 1. 题目概述

给定 `Views` 表，每行记录某人在某天浏览了某位作者的某篇文章。编写 SQL 查询，找出**所有浏览过自己文章的作者**（即 `author_id = viewer_id`）。结果按作者 `id` 升序排列，且不包含重复。
