...
- **题目名称**：最低成本联通所有城市
- **链接**：[1135. 最低成本联通所有城市](https://leetcode.cn/problems/connecting-cities-with-minimum-cost/)
- **难度**：中等
- **标签**：图、最小生成树、Kruskal、并查集、贪心

## 1. 题目概述

有 `n` 座城市，编号 `1` 到 `n`。给定一个数组 `connections`，其中 `connections[i] = [u_i, v_i, cost_i]` 表示在城市 `u_i` 与 `v_i` 之间铺设一条管道的费用为 `cost_i`。请返回**联通所有城市所需的最小总费用**；若无法将所有城市联通，返回 `-1`。
