...
- **题目名称**：矩阵中1的最大数量
- **链接**：[1183. 矩阵中 1 的最大数量](https://leetcode.cn/problems/maximum-number-of-ones/)
- **难度**：困难
- **标签**：贪心、数学、排序、堆（优先队列）

## 1. 题目概述

给定一个 `width × height` 的 0/1 矩阵 `M`，要求：任意一个 `sideLength × sideLength` 的**正方形子矩阵**中 1 的个数不超过 `maxOnes`。返回整个矩阵 `M` 中 1 的个数的最大值。
