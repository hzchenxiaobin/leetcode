...
- **题目名称**：报告的记录 II
- **链接**：[1132. 报告的记录 II](https://leetcode.cn/problems/reported-posts-ii/)
- **难度**：中等
- **标签**：SQL、LEFT JOIN、GROUP BY、COUNT(DISTINCT)、比率聚合、AVG、ROUND

## 1. 题目概述

给定 `Actions` 表（记录用户对帖子的各类操作）与 `Removals` 表（记录被删除的帖子），编写 SQL 查询，求**每天被举报为 `spam` 的帖子中、最终被删除的占比的日均值**，结果四舍五入到 2 位小数。
