...
- **题目名称**：每月交易 I
- **链接**：[1193. 每月交易 I](https://leetcode.cn/problems/monthly-transactions-i/)
- **难度**：中等
- **标签**：SQL、GROUP BY、条件聚合、SUM(IF)、DATE_FORMAT、日期截断

## 1. 题目概述

给定 `Transactions` 表，记录每笔交易的国家、状态（`approved`/`declined`）、金额和日期。编写 SQL 查询，找出**每个月、每个国家/地区**的：事务数及其总金额、已批准的事务数及其总金额。以任意顺序返回结果。
