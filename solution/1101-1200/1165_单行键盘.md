...
- **题目名称**：单行键盘
- **链接**：[1165. 单行键盘](https://leetcode.cn/problems/single-row-keyboard/)
- **难度**：简单
- **标签**：哈希表、字符串、模拟

## 1. 题目概述

有一个特殊键盘，所有按键排列在**同一行**中。给定一个长度为 26 的字符串 `keyboard`，表示键盘上从左到右各键的布局（下标从 0 到 25）。初始时手指位于下标 0 处。要输入一个字符，需要将手指移动到该字符所在的下标，移动代价为 $|i - j|$（从下标 $i$ 到下标 $j$）。
