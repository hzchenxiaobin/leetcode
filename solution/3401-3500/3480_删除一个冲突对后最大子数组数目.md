...
- **题目名称**：删除一个冲突对后最大子数组数目
- **链接**：[3480. 删除一个冲突对后最大子数组数目](https://leetcode.cn/problems/maximize-subarrays-after-removing-one-conflicting-pair/)
- **难度**：困难
- **标签**：数组、枚举、贡献法、线段树

## 1. 题目概述

给定整数 `n`，视为数组 `nums = [1, 2, ..., n]`（**值即下标**）。再给定二维数组 `conflictingPairs`，其中 `conflictingPairs[i] = [a, b]` 表示 `a` 和 `b` 构成一个**冲突对**。
