...
- **题目名称**：使每一列严格递增的最少操作次数
- **链接**：[3402. 使每一列严格递增的最少操作次数](https://leetcode.cn/problems/minimum-operations-to-make-columns-strictly-increasing/)
- **难度**：简单
- **标签**：贪心、数组、矩阵

## 1. 题目概述

给你一个由**非负整数**组成的 $m \times n$ 矩阵 `grid`。一次操作可以把任意元素 `grid[i][j]` 的值增加 1。
