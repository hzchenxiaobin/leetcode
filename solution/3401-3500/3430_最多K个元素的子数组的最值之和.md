...
- **题目名称**：最多K个元素的子数组的最值之和
- **链接**：[3430. 最多 K 个元素的子数组的最值之和](https://leetcode.cn/problems/maximum-and-minimum-sums-of-at-most-size-k-subarrays/)
- **难度**：困难
- **标签**：单调栈、数组、数学、贡献法

## 1. 题目概述

给你一个整数数组 `nums` 和一个**正**整数 `k`，返回所有**最多含有 `k` 个元素**的子数组的**最大**元素与**最小**元素之和。
