...
- **题目名称**：找出长度为 K 的特殊子字符串
- **链接**：[3456. 找出长度为 K 的特殊子字符串](https://leetcode.cn/problems/find-special-substring-of-length-k/)
- **难度**：简单
- **标签**：字符串、双指针

## 1. 题目概述

给你一个字符串 `s` 和一个整数 `k`。判断是否存在一个长度**恰好**为 `k` 的子字符串，该子字符串需要满足以下条件：
