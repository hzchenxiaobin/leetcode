...
- **题目名称**：含最多 K 个可整除元素的子数组
- **链接**：[2261. 含最多 K 个可整除元素的子数组](https://leetcode.cn/problems/k-divisible-elements-subarrays/)
- **难度**：中等
- **标签**：数组、哈希表、枚举、字典树

## 1. 题目概述

给你一个整数数组 `nums` 和两个整数 `k` 和 `p`，找出并返回满足要求的**不同子数组**的数目，要求子数组中**最多** `k` 个可被 `p` 整除的元素。
