...
- **题目名称**：移除字母异位词后的结果数组
- **链接**：[2273. 移除字母异位词后的结果数组](https://leetcode.cn/problems/find-resultant-array-after-removing-anagrams/)
- **难度**：简单
- **标签**：数组、字符串、排序

## 1. 题目概述

给你一个下标从 **0** 开始的字符串数组 `words`，其中 `words[i]` 由小写英文字母组成。在一步操作中，可任选一个下标 `i` 并 **删除** `words[i]`，但 `i` 必须同时满足：
