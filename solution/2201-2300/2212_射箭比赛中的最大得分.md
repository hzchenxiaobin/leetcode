...
- **题目名称**：射箭比赛中的最大得分
- **链接**：[2212. 射箭比赛中的最大得分](https://leetcode.cn/problems/maximum-points-in-an-archery-competition/)
- **难度**：中等
- **标签**：位运算、枚举、回溯

## 1. 题目概述

Alice 和 Bob 进行一场射箭比赛，两人各射 `numArrows` 支箭。箭靶有 `12` 个计分区域，编号 `0` 到 `11`，区域 `k` 的得分就是 `k` 分。Alice 在区域 `k` 射中 `aliceArrows[k]` 支，Bob 需要决定自己在每个区域射中多少支（记为 `bobArrows[k]`，总和恰为 `numArrows`）。
