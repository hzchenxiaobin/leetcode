...
- **题目名称**：司机成为乘客的次数
- **链接**：[2238. 司机成为乘客的次数](https://leetcode.cn/problems/number-of-times-a-driver-was-a-passenger/)
- **难度**：中等
- **标签**：数据库、SQL、自连接、`LEFT JOIN`、`GROUP BY`、`COUNT`、LeetCode 锁题

## 1. 题目概述

给定 `Rides` 表，每行记录一次行程：谁开车（`driver_id`）、谁乘车（`passenger_id`）。编写解决方案，获取**每个司机**的 ID 和他们**作为乘客**出现的次数（未当过乘客则计 0）。以**任意顺序**返回结果。
