...
- **题目名称**：加密解密字符串
- **链接**：[2227. 加密解密字符串](https://leetcode.cn/problems/encrypt-and-decrypt-strings/)
- **难度**：困难
- **标签**：设计、哈希表、字符串

## 1. 题目概述

给定字符数组 `keys`（互不相同）、字符串数组 `values`（每个长度恰为 2）与字符串数组 `dictionary`（解密后所有允许的原串）。请实现 `Encrypter` 类：
