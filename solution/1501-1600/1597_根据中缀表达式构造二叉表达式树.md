...
- **题目名称**：根据中缀表达式构造二叉表达式树
- **链接**：[1597. 根据中缀表达式构造二叉表达式树](https://leetcode.cn/problems/build-binary-expression-tree-from-infix-expression/)
- **难度**：困难
- **标签**：栈、表达式解析、二叉树、Shunting-Yard 算法

## 1. 题目概述

给定一个表示**中缀表达式**的字符串 `s`，其中包含数字（`0-9`）、四则运算符（`+`、`-`、`*`、`/`）和括号（`(`、`)`）。请构造一棵**二叉表达式树**，满足：
