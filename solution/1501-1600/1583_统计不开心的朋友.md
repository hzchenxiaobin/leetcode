...
- **题目名称**：统计不开心的朋友
- **链接**：[1583. 统计不开心的朋友](https://leetcode.cn/problems/count-unhappy-friends/)
- **难度**：中等
- **标签**：数组、模拟、预处理、哈希表

## 1. 题目概述

给定 `n` 位朋友（`n` 为偶数）的亲近程度列表 `preferences`，其中 `preferences[i]` 是按**亲近程度从高到低**排列的朋友列表（不含 `i` 本身）。所有朋友被分成 `n/2` 对，配对情况由 `pairs` 给出，`pairs[i] = [xi, yi]` 表示 `xi` 与 `yi` 配对。
