...
- **题目名称**：矩阵的最大非负积
- **链接**：[1594. 矩阵的最大非负积](https://leetcode.cn/problems/max-non-negative-product-in-a-matrix/)
- **难度**：中等
- **标签**：数组、动态规划、矩阵

## 1. 题目概述

给定一个 `rows × cols` 的矩阵 `grid`。你从 `(0, 0)` 出发，每步只能**向右**或**向下**移动，最终到达 `(rows-1, cols-1)`。路径的**积**为路径上所有元素相乘的结果。
