...
- **题目名称**：每位顾客最经常订购的商品
- **链接**：[1596. 每位顾客最经常订购的商品 🔒](https://leetcode.cn/problems/the-most-frequently-ordered-products-for-each-customer/)
- **难度**：中等
- **标签**：数据库、SQL、窗口函数、`RANK`、Top-1 per group、并列保留、`GROUP BY` + `COUNT`、聚合后排名、`JOIN`、`MAX` + `IN` 子查询、LeetCode 锁题

## 1. 题目概述

给定 `Customers`、`Orders`、`Products` 三张表，编写 SQL 查询，找到**每位顾客最经常订购的商品**。若同一顾客有多个商品并列最高订购次数，则**全部返回**；无订单的顾客不出现在结果中。结果可按任意顺序返回。
