...
- **题目名称**：图中最相似的路径
- **链接**：[1548. 图中最相似的路径](https://leetcode.cn/problems/the-most-similar-path-in-a-graph/)
- **难度**：困难
- **标签**：图、动态规划、字符串、数组

## 1. 题目概述

有 `n` 座城市和 `m` 条**双向道路**，`roads[i] = [ai, bi]` 连接城市 `ai` 与 `bi`。每座城市的名字恰为 3 个大写英文字母，存于字符串数组 `names`。整张图保证**连通**且任意两城之间至多一条直达道路。
