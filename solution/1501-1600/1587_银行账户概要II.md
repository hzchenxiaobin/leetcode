...
- **题目名称**：银行账户概要 II
- **链接**：[1587. 银行账户概要 II](https://leetcode.cn/problems/bank-account-summary-ii/)
- **难度**：简单
- **标签**：数据库、SQL、`JOIN`、`GROUP BY`、`SUM`、`HAVING`、聚合后过滤

## 1. 题目概述

给定 `Users`（账户名册）与 `Transactions`（交易流水）两张表，编写 SQL 查询，报告**余额高于 10000 的所有用户的名字和余额**。账户的余额等于包含该账户的所有交易的总和，所有账户起始余额为 0。
