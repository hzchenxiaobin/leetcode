...
- **题目名称**：好叶子节点对的数量
- **链接**：[1530. 好叶子节点对的数量](https://leetcode.cn/problems/number-of-good-leaf-nodes-pairs/)
- **难度**：中等
- **标签**：树、二叉树、深度优先搜索、后序遍历

## 1. 题目概述

给你二叉树的根节点 `root` 和一个整数 `distance`。如果二叉树中两个**叶**节点之间的**最短路径长度**小于或等于 `distance`，那它们就可以构成一组**好叶子节点对**。返回树中好叶子节点对的数量。
