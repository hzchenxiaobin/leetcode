...
- **题目名称**：银行账户概要
- **链接**：[1555. 银行账户概要 🔒](https://leetcode.cn/problems/bank-account-summary/)
- **难度**：中等
- **标签**：SQL、UNION ALL、GROUP BY、聚合求和、多表关联、条件判定、LeetCode 锁题

## 1. 题目概述

力扣银行（LCB）记录每一条虚拟支付交易。给定 `Users`（用户当前额度）与 `Transactions`（交易流水）两张表，编写 SQL 查询，报告**每个用户完成所有交易后的当前余额**，并判定是否**透支**（余额 $< 0$）。
