...
- **题目名称**：圆形赛道上经过次数最多的扇区
- **链接**：[1560. 圆形赛道上经过次数最多的扇区](https://leetcode.cn/problems/most-visited-sector-in-a-circular-track/)
- **难度**：简单
- **标签**：数组、模拟、数学

## 1. 题目概述

给你一个整数 `n` 和一个整数数组 `rounds`。有一条圆形赛道由 `n` 个扇区组成，扇区编号从 `1` 到 `n`。马拉松全程由 `m` 个阶段组成，第 `i` 个阶段从扇区 `rounds[i-1]` 开始，到扇区 `rounds[i]` 结束。赛道按扇区编号升序逆时针形成圆。请返回经过次数最多的扇区，按编号升序排列。
