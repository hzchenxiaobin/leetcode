...
- **题目名称**：子数组和排序后的区间和
- **链接**：[1508. 子数组和排序后的区间和](https://leetcode.cn/problems/range-sum-of-sorted-subarray-sums/)
- **难度**：中等
- **标签**：数组、二分查找、前缀和、滑动窗口

## 1. 题目概述

给你一个正整数数组 `nums`（长度为 `n`），计算**所有非空连续子数组的和**，将它们**升序排序**得到一个长度为 $n(n+1)/2$ 的新数组。返回新数组中下标从 `left` 到 `right`（**1-indexed**，含两端）的元素之和，并对 $10^9+7$ 取模。
