...
- **题目名称**：患某种疾病的患者
- **链接**：[1527. 患某种疾病的患者](https://leetcode.cn/problems/patients-with-a-condition/)
- **难度**：简单
- **标签**：SQL、`LIKE`、`REGEXP`、模式匹配

## 1. 题目概述

给定 `Patients` 表，编写 SQL 查询，查找患有 **I 类糖尿病**的患者。I 类糖尿病的疾病代码总是包含前缀 `DIAB1`。`conditions` 字段包含 0 个或多个疾病代码，以**空格分隔**。
