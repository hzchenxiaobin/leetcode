...
- **题目名称**：按月统计订单数与顾客数
- **链接**：[1565. 按月统计订单数与顾客数 🔒](https://leetcode.cn/problems/unique-orders-and-customers-per-month/)
- **难度**：简单
- **标签**：数据库、SQL、`GROUP BY`、`COUNT`、`COUNT(DISTINCT)`、`DATE_FORMAT`、`WHERE` 行级过滤、LeetCode 锁题

## 1. 题目概述

给定 `Orders` 表，编写 SQL 查询，**按月**统计金额（`invoice`）**大于 $20** 的唯一**订单数**和唯一**顾客数**。查询结果无排序要求。
