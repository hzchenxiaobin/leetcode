...
- **题目名称**：判断能否形成等差数列
- **链接**：[1502. 判断能否形成等差数列](https://leetcode.cn/problems/can-make-arithmetic-progression-from-sequence/)
- **难度**：简单
- **标签**：数组、排序

## 1. 题目概述

给你一个数字数组 `arr`。如果一个数列中，任意相邻两项的差总等于同一个常数，那么这个数列就称为**等差数列**。如果可以重新排列数组形成等差数列，请返回 `true`；否则返回 `false`。
