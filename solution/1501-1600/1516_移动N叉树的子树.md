...
- **题目名称**：移动 N 叉树的子树
- **链接**：[1516. 移动 N 叉树的子树](https://leetcode.cn/problems/move-sub-tree-of-n-ary-tree/)
- **难度**：困难
- **标签**：树、N 叉树、深度优先搜索

## 1. 题目概述

给定一棵**节点值唯一**的 N 叉树的根节点 `root`，以及树中两个不同节点 `p` 和 `q`。你需要将 `p` 的子树移动为 `q` 的**最后一个直接子节点**。如果 `p` 已经是 `q` 的直接子节点，则不做任何改变。
