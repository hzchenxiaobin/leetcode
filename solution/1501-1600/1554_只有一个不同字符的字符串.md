...
- **题目名称**：只有一个不同字符的字符串
- **链接**：[1554. 只有一个不同字符的字符串](https://leetcode.cn/problems/strings-differ-by-one-character/)
- **难度**：中等
- **标签**：哈希表、字符串、滚动哈希、Rabin-Karp

## 1. 题目概述

给定一个字符串列表 `dict`，其中所有字符串**长度相同**且均由小写字母组成。判断是否存在**一对字符串**，它们在**恰好一个位置**上的字符不同（其余位置完全相同）。存在则返回 `true`，否则返回 `false`。
