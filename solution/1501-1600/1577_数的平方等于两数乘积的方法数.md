...
- **题目名称**：数的平方等于两数乘积的方法数
- **链接**：[1577. 数的平方等于两数乘积的方法数](https://leetcode.cn/problems/number-of-ways-where-square-of-number-is-equal-to-product-of-two-numbers/)
- **难度**：中等
- **标签**：数组、哈希表、数学、计数

## 1. 题目概述

给定两个整数数组 `nums1` 和 `nums2`，返回满足以下规则的三元组数目（类型 1 + 类型 2）：
