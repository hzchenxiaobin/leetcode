...
- **题目名称**：统计点对的数目
- **链接**：[1782. 统计点对的数目](https://leetcode.cn/problems/count-pairs-of-nodes/)
- **难度**：困难
- **标签**：图、排序、双指针、哈希表

## 1. 题目概述

给定一个无向图，由整数 `n`（节点数目）和 `edges` 组成，`edges[i] = [ui, vi]` 表示 `ui` 和 `vi` 之间有一条无向边，**图中可能存在多重边**。再给定查询数组 `queries`。
