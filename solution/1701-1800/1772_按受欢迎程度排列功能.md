...
- **题目名称**：按受欢迎程度排列功能
- **链接**：[1772. 按受欢迎程度排列功能](https://leetcode.cn/problems/sort-features-by-popularity/)
- **难度**：中等
- **标签**：数组、哈希表、字符串、排序

## 1. 题目概述

给定一个字符串数组 `features`，其中 `features[i]` 是一个单词，描述一个功能的名称。另给定一个字符串数组 `responses`，其中 `responses[i]` 是一个以空格分隔的单词串，表示某位用户提及的功能。
