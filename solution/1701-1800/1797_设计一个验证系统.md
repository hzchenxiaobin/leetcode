...
- **题目名称**：设计一个验证系统
- **链接**：[1797. Design Authentication Manager](https://leetcode.cn/problems/design-authentication-manager/)
- **难度**：中等
- **标签**：设计、哈希表、堆、惰性删除

## 1. 题目概述

设计一个验证码验证系统。每个验证码在生成时刻 `currentTime` 之后 `timeToLive` 秒过期。验证码可被续期（延长 `timeToLive` 秒）。
