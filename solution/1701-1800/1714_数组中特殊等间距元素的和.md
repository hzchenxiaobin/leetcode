...
- **题目名称**：数组中特殊等间距元素的和
- **链接**：[1714. 数组中特殊等间距元素的和](https://leetcode.cn/problems/sum-of-special-evenly-spaced-elements-in-array/)
- **难度**：困难
- **标签**：数组、根号分治（Sqrt Decomposition）、动态规划、前缀和

## 1. 题目概述

给定一个下标从 $0$ 开始的整数数组 `nums`（长度为 $n$），和一个二维查询数组 `queries`。每个查询 $\text{queries}[i] = [x_i,\ y_i]$ 要求计算从下标 $x_i$ 出发、**每隔步长 $y_i$** 取一个元素求和：
