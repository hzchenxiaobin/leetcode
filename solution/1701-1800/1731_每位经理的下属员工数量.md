...
- **题目名称**：每位经理的下属员工数量
- **链接**：[1731. 每位经理的下属员工数量](https://leetcode.cn/problems/the-number-of-employees-which-report-to-each-employee/)
- **难度**：简单
- **标签**：数据库、SQL、自连接、`GROUP BY`、`COUNT`、`AVG`、`ROUND`、`ORDER BY`

## 1. 题目概述

给定 `Employees` 表（员工 ID、姓名、直属上级 `reports_to`、年龄）。`reports_to` 为 `null` 表示该员工不需要向任何人汇报。题目约定**至少有一个其他员工向他汇报的人即为「经理」**。要求返回每位经理的：
