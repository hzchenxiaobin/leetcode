...
- **题目名称**：员工的直属部门
- **链接**：[1789. 员工的直属部门](https://leetcode.cn/problems/primary-department-for-each-employee/)
- **难度**：简单
- **标签**：数据库、SQL、`UNION`、`GROUP BY`、`HAVING`、`WHERE`、窗口函数、`COUNT() OVER`

## 1. 题目概述

给定 `Employee` 表，每行记录一名员工与一个部门的关联关系，以及该部门是否为该员工的直属部门（`primary_flag`，枚举 `'Y'`/`'N'`）。一个员工可以属于多个部门。规则如下：
