...
- **题目名称**：生成乘积数组的方案数
- **链接**：[1735. 生成乘积数组的方案数](https://leetcode.cn/problems/count-ways-to-make-array-with-product/)
- **难度**：困难
- **标签**：数学、数论、组合数学、质因数分解、预处理

## 1. 题目概述

给定一个二维整数数组 `queries`，其中 `queries[i] = [ni, ki]`。第 $i$ 个查询要求构造一个长度为 $n_i$、每个元素都是正整数的数组，且满足所有元素的**乘积**为 $k_i$。求有多少种可行方案。由于答案可能很大，需对 $10^9 + 7$ **取余**。
