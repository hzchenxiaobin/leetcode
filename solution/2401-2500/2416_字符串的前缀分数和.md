...
- **题目名称**：字符串的前缀分数和
- **链接**：[2416. 字符串的前缀分数和](https://leetcode.cn/problems/sum-of-prefix-scores-of-strings/)
- **难度**：困难
- **标签**：字典树、数组、字符串、前缀

## 1. 题目概述

给定一个长度为 `n` 的非空字符串数组 `words`。定义字符串 `term` 的**分数**等于 `words` 中以 `term` 作为**前缀**的字符串的数目（字符串视作它自身的一个前缀）。
