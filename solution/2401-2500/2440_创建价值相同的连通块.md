...
- **题目名称**：创建价值相同的连通块
- **链接**：[2440. 创建价值相同的连通块](https://leetcode.cn/problems/create-components-with-same-value/)
- **难度**：困难
- **标签**：树、深度优先搜索、数学、枚举

## 1. 题目概述

有一棵 $n$ 个节点（编号 $0 \dots n-1$）的**无向树**（$n-1$ 条边），每个节点 $i$ 有权值 $\text{nums}[i]$。你可以**删除若干边**，把树切成若干连通块；一个连通块的**价值** = 块内所有节点权值之和。要求删除后**各连通块价值都相等**，求最多能删多少条边（等价于求最多能切出多少块）。
