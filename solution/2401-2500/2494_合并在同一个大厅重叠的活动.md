...
- **题目名称**：合并在同一个大厅重叠的活动
- **链接**：[2494. 合并在同一个大厅重叠的活动](https://leetcode.cn/problems/merge-overlapping-events-in-the-same-hall/)
- **难度**：困难
- **标签**：数据库、SQL、窗口函数、`Gaps and Islands`、区间合并、`MAX OVER`、LeetCode 锁题

## 1. 题目概述

给定 `HallEvents` 表，每行记录某个大厅（`hall_id`）里一场活动的起止日期（`start_day`、`end_day`，含端点）。编写解决方案，把**同一大厅内互相重叠的活动**合并成一条连续区间，返回 `hall_id`、合并后的 `start_day` 与 `end_day`，按 `hall_id`、`start_day` 升序排列。
