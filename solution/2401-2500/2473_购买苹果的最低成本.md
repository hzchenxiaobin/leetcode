...
- **题目名称**：购买苹果的最低成本
- **链接**：[2473. 购买苹果的最低成本](https://leetcode.cn/problems/minimum-cost-to-buy-apples/)
- **难度**：中等
- **标签**：图、最短路、堆（优先队列）、多源 Dijkstra、虚拟源点

## 1. 题目概述

给定 $n$ 个城市（编号 $1\sim n$）与无向边数组 `roads`，其中 `roads[i] = [a_i, b_i, cost_i]` 表示 $a_i$ 与 $b_i$ 之间有一条双向道路，**去程**通行成本为 $\text{cost}_i$。每个城市 $i$ 都能买到苹果，价格为 $\text{appleCost}[i]$（数组 0 下标对应城市 $1$）。
