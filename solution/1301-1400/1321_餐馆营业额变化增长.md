...
- **题目名称**：餐馆营业额变化增长
- **链接**：[1321. 餐馆营业额变化增长](https://leetcode.cn/problems/restaurant-growth/)
- **难度**：中等
- **标签**：数据库、SQL、窗口函数、滑动窗口、ROWS BETWEEN、移动平均、GROUP BY

## 1. 题目概述

给定 `Customer` 表，每行记录一名顾客在某一天的消费总额。请编写 SQL 查询，计算以 **7 天**（某日期 + 该日期前的 6 天）为一个时间段的顾客消费**平均值**，`average_amount` 保留两位小数。结果按 `visited_on` 升序排序。
