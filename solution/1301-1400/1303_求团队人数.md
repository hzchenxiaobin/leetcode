...
- **题目名称**：求团队人数
- **链接**：[1303. 求团队人数](https://leetcode.cn/problems/find-the-team-size/)
- **难度**：简单
- **标签**：数据库、SQL、窗口函数、PARTITION BY、COUNT、自连接

## 1. 题目概述

给定 `Employee` 表，每行记录一名员工的 `emp_id` 及其所属团队 `team_id`。编写 SQL 查询，为**每个员工**返回其所在团队的人数 `team_size`，结果按 `emp_id` 升序排列。
