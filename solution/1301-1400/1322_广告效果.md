...
- **题目名称**：广告效果
- **链接**：[1322. 广告效果](https://leetcode.cn/problems/ads-performance/)
- **难度**：简单
- **标签**：数据库、SQL、条件聚合、SUM(CASE)、除零处理、IFNULL/NULLIF、CTR

## 1. 题目概述

给定 `Ads` 表，每行记录一个用户对一条广告采取的行为。`action` 是枚举类型，取值为 `'Clicked'`（点击）、`'Viewed'`（浏览）、`'Ignored'`（忽略）。公司希望计算每条广告的**点击通过率（Click-Through Rate, CTR）**，衡量广告效果。CTR 公式如下：
