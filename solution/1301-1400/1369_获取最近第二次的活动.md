...
- **题目名称**：获取最近第二次的活动
- **链接**：[1369. 获取最近第二次的活动](https://leetcode.cn/problems/get-the-second-most-recent-activity/)
- **难度**：困难
...
给定 `UserActivity` 表，每行记录某用户在一段时间 `[startDate, endDate]` 内从事的一项活动。请编写 SQL 查询，报告**每个用户的「最近第二次」活动**（即按时间倒序排的第 2 条）。**若某用户只有一条活动记录，则直接返回那一条。** 结果顺序任意。
