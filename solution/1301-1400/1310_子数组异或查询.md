...
- **题目名称**：子数组异或查询
- **链接**：[1310. 子数组异或查询](https://leetcode.cn/problems/xor-queries-of-a-subarray/)
- **难度**：中等
- **标签**：位运算、数组、前缀和

## 1. 题目概述

有一个正整数数组 `arr`，现给出查询数组 `queries`，其中 `queries[i] = [L_i, R_i]`。对每个查询，计算 `arr[L_i] xor arr[L_i+1] xor ... xor arr[R_i]` 的值，返回包含所有查询结果的数组。
