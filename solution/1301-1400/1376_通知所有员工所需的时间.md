...
- **题目名称**：通知所有员工所需的时间
- **链接**：[1376. 通知所有员工所需的时间](https://leetcode.cn/problems/time-needed-to-inform-all-employees/)
- **难度**：中等
- **标签**：树、深度优先搜索、广度优先搜索

## 1. 题目概述

公司里有 `n` 名员工，编号 `0` 到 `n - 1`，ID 唯一。总负责人由 `headID` 标识。`manager[i]` 是第 `i` 名员工的直属负责人，`manager[headID] = -1`，题目保证从属关系构成一棵**树**。
