...
- **题目名称**：可获得的最大点数
- **链接**：[1423. 可获得的最大点数](https://leetcode.cn/problems/maximum-points-you-can-obtain-from-cards/)
- **难度**：中等
- **标签**：数组、滑动窗口、前缀和

## 1. 题目概述

几张卡牌排成一行，每张卡牌都有一个对应的点数（整数 `cardPoints[i]`）。每次行动可以从行的**开头**或**末尾**拿一张卡牌，**共拿 `k` 张**。你的得分是你拿到的卡牌点数之和。返回可以获得的最大分数。
