...
- **题目名称**：粉刷房子 III
- **链接**：[1473. 粉刷房子 III](https://leetcode.cn/problems/paint-house-iii/)
- **难度**：困难
- **标签**：动态规划、划分型 DP、状态机 DP

## 1. 题目概述

一排 `m` 个房子，每个房子要涂成 `n` 种颜色之一（颜色编号 `1` 到 `n`）。部分房子去年已涂色（`houses[i] != 0`），**不可重涂**；`houses[i] == 0` 表示尚未涂色。`cost[i][j]` 是把第 `i` 个房子涂成颜色 `j+1` 的花费。
