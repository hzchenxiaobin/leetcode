...
- **题目名称**：活跃用户
- **链接**：[1454. 活跃用户 🔒](https://leetcode.cn/problems/active-users/)
- **难度**：中等
- **标签**：数据库、SQL、窗口函数、`ROW_NUMBER`、`DATE_SUB`/`DATEDIFF`、连续区间分组、`GROUP BY` + `HAVING`、`DISTINCT`、`JOIN`、LeetCode 锁题

## 1. 题目概述

给定 `Accounts` 表与 `Logins` 表。`Accounts` 每行记录一个账户（`id`、`name`）；`Logins` 每行记录一次登录（`id`、`login_date`）。**活跃用户**是指那些**至少连续 5 天登录账户**的用户。请编写 SQL，找到活跃用户的 `id` 和 `name`，结果按 `id` 升序排列。
