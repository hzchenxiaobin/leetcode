...
- **题目名称**：查找成绩处于中游的学生
- **链接**：[1412. 查找成绩处于中游的学生](https://leetcode.cn/problems/find-the-quiet-students-in-all-exams/)
- **难度**：困难
- **标签**：SQL、GROUP BY、MAX、MIN、JOIN、NOT IN、子查询、集合差

## 1. 题目概述

给定 `Student` 和 `Exam` 两张表。`Student` 记录学生 ID 与姓名，`Exam` 记录每场考试中各学生的分数。**中游学生**（quiet student）满足以下两个条件：
