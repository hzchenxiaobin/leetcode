...
- **题目名称**：至少有一个 1 的最左端列
- **链接**：[1428. 至少有一个 1 的最左端列](https://leetcode.cn/problems/leftmost-column-with-at-least-a-one/)
- **难度**：中等
- **标签**：数组、二分查找、矩阵、交互题

## 1. 题目概述

本题是一道**交互题**。给定一个**二进制矩阵** `BinaryMatrix`，其中每个元素为 `0` 或 `1`，且**每一行按非递减顺序排序**（即每行形如 `0..0 1..1`，前缀 0、后缀 1）。请返回**最左端含有至少一个 1 的列的下标**；若整个矩阵没有 1，返回 `-1`。
