...
- **题目名称**：满足条件的子序列数目
- **链接**：[1498. 满足条件的子序列数目](https://leetcode.cn/problems/number-of-subsequences-that-satisfy-the-given-sum-condition/)
- **难度**：中等
- **标签**：数组、双指针、二分查找、排序、贪心

## 1. 题目概述

给你一个整数数组 `nums` 和一个整数 `target`。请统计并返回 `nums` 中能满足其**最小元素与最大元素之和小于或等于 `target`** 的**非空子序列**数目。由于答案可能很大，将结果对 $10^9 + 7$ 取余后返回。
