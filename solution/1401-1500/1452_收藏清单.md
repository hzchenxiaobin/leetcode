...
- **题目名称**：收藏清单
- **链接**：[1452. 收藏清单](https://leetcode.cn/problems/people-whose-list-of-favorite-companies-is-not-a-subset-of-another-list/)
- **难度**：中等
- **标签**：位运算、哈希表、字符串、排序

## 1. 题目概述

给你一个数组 `favoriteCompanies`，其中 `favoriteCompanies[i]` 是第 $i$ 名用户收藏的公司清单（下标从 0 开始）。请找出**不是其他任何人收藏的公司清单的子集**的收藏清单，返回该清单下标，下标需按升序排列。
