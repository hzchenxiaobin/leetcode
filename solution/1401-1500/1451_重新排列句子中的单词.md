...
- **题目名称**：重新排列句子中的单词
- **链接**：[1451. 重新排列句子中的单词](https://leetcode.cn/problems/rearrange-words-in-a-sentence/)
- **难度**：中等
- **标签**：字符串、排序、稳定排序

## 1. 题目概述

给定一个满足下述格式的句子 `text`：首字母大写，每个单词用单个空格分隔。请重新排列 `text` 中的单词，使所有单词按其**长度的升序**排列；若两个单词长度相同，则**保留其在原句子中的相对顺序**。返回时同样首字母大写、其余小写、单词间单空格分隔。
