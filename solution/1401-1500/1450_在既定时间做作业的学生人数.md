...
给你两个整数数组 `startTime`（开始时间）和 `endTime`（结束时间），并指定一个整数 `queryTime` 作为查询时间。第 `i` 名学生在 `startTime[i]` 时开始写作业并于 `endTime[i]` 时完成作业。请返回在查询时间 `queryTime` 时**正在做作业**的学生人数。形式上，返回满足 `queryTime` 处于闭区间 `[startTime[i], endTime[i]]`（含两端）的学生人数。
