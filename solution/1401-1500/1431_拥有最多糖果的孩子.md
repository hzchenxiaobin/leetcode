...
- **题目名称**：拥有最多糖果的孩子
- **链接**：[1431. 拥有最多糖果的孩子](https://leetcode.cn/problems/kids-with-the-greatest-number-of-candies/)
- **难度**：简单
- **标签**：数组、模拟、预处理最大值

## 1. 题目概述

有 `n` 个孩子，第 `i` 个孩子拥有 `candies[i]` 颗糖果。另有整数 `extraCandies` 表示你手里的额外糖果。对于每个孩子，判断：**若把全部 `extraCandies` 都给他**，他是否拥有**不少于**任何其他孩子的糖果数（即「拥有最多糖果的孩子」之一，并列最多也算）。返回一个长度为 `n` 的布尔列表 `answer`，`answer[i] = true` 表示第 `i` 个孩子给糖后能成为最多。
