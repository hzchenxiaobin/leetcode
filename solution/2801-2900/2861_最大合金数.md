...
- **题目名称**：最大合金数
- **链接**：[2861. 最大合金数](https://leetcode.cn/problems/maximum-number-of-alloys/)
- **难度**：中等
- **标签**：数组、二分查找

## 1. 题目概述

你是合金制造公司的老板：共有 `n` 种金属和 `k` 台机器。第 `i` 台机器制造一份合金需要 `composition[i][j]` 份第 `j` 种金属。最初你拥有 `stock[j]` 份第 `j` 种金属，每购入一份第 `j` 种金属需要花费 `cost[j]` 的金钱。
