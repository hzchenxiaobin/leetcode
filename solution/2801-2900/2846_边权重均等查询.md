...
- **题目名称**：边权重均等查询
- **链接**：[2846. 边权重均等查询](https://leetcode.cn/problems/minimum-edge-weight-equilibrium-queries-in-a-tree/)
- **难度**：困难
- **标签**：树、深度优先搜索、最近公共祖先、倍增、前缀和

## 1. 题目概述

给定一棵由 `n` 个节点组成的无向树（节点编号 `0` 到 `n-1`），`edges[i] = [u_i, v_i, w_i]` 表示节点 `u_i` 与 `v_i` 之间存在一条权值为 `w_i` 的边。另有 `m` 条查询 `queries[i] = [a_i, b_i]`：对每条查询，求使 `a_i` 到 `b_i` 路径上**每条边的权值相等**所需的最小操作次数。一次操作可以选择树上任意一条边，并将其权值改为任意值。
