...
- **题目名称**：统计和小于目标下标对数目
- **链接**：[2824. 统计和小于目标下标对数目](https://leetcode.cn/problems/count-pairs-whose-sum-is-less-than-target/)
- **难度**：简单
- **标签**：数组、双指针、二分查找、排序

## 1. 题目概述

给你一个下标从 **0** 开始长度为 $n$ 的整数数组 `nums` 和一个整数 `target`，请你返回满足 $0 \le i < j < n$ 且 $\text{nums}[i] + \text{nums}[j] < \text{target}$ 的下标对 $(i, j)$ 的数目。
