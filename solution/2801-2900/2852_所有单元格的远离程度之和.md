...
- **题目名称**：所有单元格的远离程度之和
- **链接**：[2852. 所有单元格的远离程度之和](https://leetcode.cn/problems/sum-of-remoteness-of-all-cells/)
- **难度**：中等
...
给定一个下标从 **0** 开始、大小为 `n * n` 的矩阵 `grid`，其中每个单元格的值 `grid[i][j]` 要么是**正整数**，要么是 `-1` 表示该格**阻塞**。你可以从一个非阻塞单元格移动到与其共享边的任意非阻塞单元格（四方向，不许穿越阻塞格）。
