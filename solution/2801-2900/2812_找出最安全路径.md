...
- **题目名称**：找出最安全路径
- **链接**：[2812. 找出最安全路径](https://leetcode.cn/problems/find-the-safest-path-in-a-grid/)
- **难度**：中等
- **标签**：广度优先搜索、并查集、二分查找、矩阵、堆（优先队列）

## 1. 题目概述

给你一个下标从 **0** 开始、大小为 `n x n` 的矩阵 `grid`，其中 `grid[r][c] = 1` 表示该格有小偷，`grid[r][c] = 0` 表示空格。你从 `(0, 0)` 出发，每步可移动到上下左右任一相邻单元格（含小偷格）。
