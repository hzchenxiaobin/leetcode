...
- **题目名称**：自定义间隔
- **链接**：[2805. Custom Interval](https://leetcode.cn/problems/custom-interval/)
- **难度**：中等
- **标签**：数学、模拟、二分查找、定时器
...
实现一个函数 `foobar(delay, period, cancelTime)`，模拟一种**自定义间隔**的定时调度：与 `setInterval` 的固定间隔不同，这里的调用间隔会**逐次递增**。具体规则为：
