...
- **题目名称**：有序链表转换二叉搜索树
- **链接**：[109. 有序链表转换二叉搜索树](https://leetcode.cn/problems/convert-sorted-list-to-binary-search-tree/)
- **难度**：中等
- **标签**：树、BST、链表、分治、快慢指针

## 1. 题目概述

给定一个**升序排列**的单链表头节点 `head`，请将其转换为一棵**高度平衡**的二叉搜索树（BST）。高度平衡是指每个节点的左右两个子树的高度差不超过 1。
