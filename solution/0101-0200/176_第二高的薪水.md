...
- **题目名称**：第二高的薪水
- **链接**：[176. 第二高的薪水](https://leetcode.cn/problems/second-highest-salary/)
- **难度**：中等
- **标签**：SQL、子查询、聚合函数、LIMIT/OFFSET

## 1. 题目概述

给定一张 `Employee` 表，编写 SQL 查询，返回**第二高（Second Highest）**的薪水 `salary`。如果不存在第二高的薪水（即不同薪水值少于 2 个），查询应返回 `NULL`。
