...
> 💡 与 [Day5 Mini Engine v1](../../aiinfra/daily/week6/day5/README.md) 的请求分组同构——v1 每轮把多个请求组成一个 batch 一起 forward，类似把相邻的 `'1'` 归为同一岛屿（连通分量）。DFS 标记连通块 = Scheduler 把同轮可调度的请求归入同一 batch。
...
- [200. 岛屿数量](https://leetcode.cn/problems/number-of-islands/)（[题解](岛屿数量.md)）：DFS/BFS 计数
