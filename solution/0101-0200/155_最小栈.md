...
> 💡 与 [Day 2 KV Cache](../../../ai-infra-notes/aiinfra/daily/week5/day2/README.md) 的模式同构：KV Cache 把"历史 K/V"缓存到 `k_cache`，Decode 不必重算前缀（`O(L·d²)→O(d²)`）；最小栈把"栈内最小值"缓存到 `min_stack`，`getMin` 不必遍历栈（`O(n)→O(1)`）。两者都是**空间换时间 + 辅助状态随主数据增量维护**。
