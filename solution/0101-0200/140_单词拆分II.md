...
- **题目名称**：单词拆分 II
- **链接**：[140. 单词拆分 II](https://leetcode.cn/problems/word-break-ii/)
- **难度**：困难
- **标签**：字典树、记忆化搜索、哈希表、字符串、动态规划、回溯

## 1. 题目概述

给定一个字符串 `s` 和一个字符串字典 `wordDict`，在 `s` 中增加空格将其拆分成若干个单词，使得**每个单词都在字典中**。返回所有可能的拆分方案。每个方案是一个由单词组成的字符串（单词之间用空格连接）。
