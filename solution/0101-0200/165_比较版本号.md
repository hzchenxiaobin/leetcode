...
- **题目名称**：比较版本号
- **链接**：[165. 比较版本号](https://leetcode.cn/problems/compare-version-numbers/)
- **难度**：中等
- **标签**：字符串、双指针

## 1. 题目概述

给你两个版本字符串 `version1` 和 `version2`，请你比较它们。版本号由一个或多个**修订号**组成，修订号之间用点号 `'.'` 分隔。每个修订号可能包含**前导零**，比较时应当忽略（即 `1.01` 和 `1.001` 视为相等）。
