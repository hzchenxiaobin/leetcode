...
- **题目名称**：从给定原材料中找到所有可以做出的菜
- **链接**：[2115. 从给定原材料中找到所有可以做出的菜](https://leetcode.cn/problems/find-all-possible-recipes-from-given-supplies/)
- **难度**：中等
- **标签**：图、拓扑排序、广度优先搜索、哈希表

## 1. 题目概述

你有 `n` 道不同的菜。给定字符串数组 `recipes` 和二维字符串数组 `ingredients`，第 `i` 道菜名为 `recipes[i]`，所需全部原料为 `ingredients[i]`。**一道菜也可以是另一道菜的原料**——即 `ingredients[i]` 中可能包含 `recipes` 里的另一个字符串。再给你初始拥有的原料数组 `supplies`（每种无限多）。返回你能做出的所有菜，顺序任意。
