...
- **题目名称**：分组得分最高的所有下标
- **链接**：[2155. 分组得分最高的所有下标](https://leetcode.cn/problems/all-divisions-with-the-highest-score-of-a-binary-array/)
- **难度**：中等
- **标签**：数组、前缀和、一次遍历

## 1. 题目概述

给定下标从 `0` 开始的二进制数组 `nums`，长度为 `n`。可按下标 `i`（$0 \le i \le n$）拆成两段（均可为空）：
