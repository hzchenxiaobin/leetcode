...
- **题目名称**：根据描述创建二叉树
- **链接**：[2196. 根据描述创建二叉树](https://leetcode.cn/problems/create-binary-tree-from-descriptions/)
- **难度**：中等
- **标签**：树、二叉树、哈希表、构造

## 1. 题目概述

给你一个二维整数数组 `descriptions`，其中 `descriptions[i] = [parenti, childi, isLefti]` 表示 `parenti` 是 `childi` 在二叉树中的父节点，二叉树中各节点的值 **互不相同**。`isLefti == 1` 表示 `childi` 是左子节点，`isLefti == 0` 表示右子节点。请根据描述构造二叉树并返回其根节点。测试用例保证可以构造出有效的二叉树。
