...
- **题目名称**：通过翻转行或列来去除所有的 1 II
- **链接**：[2174. 通过翻转行或列来去除所有的 1 II](https://leetcode.cn/problems/remove-all-ones-with-row-and-column-flips-ii/)
- **难度**：中等
- **标签**：位运算、广度优先搜索、数组、矩阵

## 1. 题目概述

给定一个下标从 **0** 开始的 $m \times n$ **二进制**矩阵 `grid`。在一次操作中，可以选择满足以下条件的任意 $i$ 和 $j$：
