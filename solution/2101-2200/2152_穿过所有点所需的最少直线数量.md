...
- **题目名称**：穿过所有点所需的最少直线数量
- **链接**：[2152. 穿过所有点所需的最少直线数量](https://leetcode.cn/problems/minimum-number-of-lines-to-cover-points/)
- **难度**：中等
- **标签**：位运算、几何、动态规划、状态压缩、回溯、数组、哈希表、数学

## 1. 题目概述

给定平面上 `n` 个**互不相同**的点 `points`，其中 `points[i] = [xi, yi]`。一条直线「覆盖」一个点，当且仅当该点落在这条直线上。返回**能够覆盖所有给定点**的**最少直线数目**。
