...
- **题目名称**：将杂乱无章的数字排序
- **链接**：[2191. 将杂乱无章的数字排序](https://leetcode.cn/problems/sort-the-jumbled-numbers/)
- **难度**：中等
- **标签**：数组、排序、模拟

## 1. 题目概述

给定一个下标从 0 开始的整数数组 `mapping`（长度为 10），表示十进制数位的映射规则：`mapping[i] = j` 表示将数位 `i` 映射为数位 `j`。`mapping` 是 0–9 的一个排列。
