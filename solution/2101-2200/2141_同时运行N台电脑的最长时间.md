...
- **题目名称**：同时运行 N 台电脑的最长时间
- **链接**：[2141. 同时运行 N 台电脑的最长时间](https://leetcode.cn/problems/maximum-running-time-of-n-computers/)
- **难度**：困难
- **标签**：数组、二分查找、贪心

## 1. 题目概述

你有 `n` 台电脑，给定下标从 0 开始的整数数组 `batteries`，其中第 `i` 个电池可让一台电脑**运行** `batteries[i]` 分钟。要求使用这些电池让**全部** `n` 台电脑**同时**运行。
