...
- **题目名称**：统计可以被K整除的下标对数目
- **链接**：[2183. 统计可以被 K 整除的下标对数目](https://leetcode.cn/problems/count-array-pairs-divisible-by-k/)
- **难度**：困难
- **标签**：数组、数学、数论、哈希表、枚举

## 1. 题目概述

给定下标从 0 开始、长度为 `n` 的整数数组 `nums` 和一个整数 `k`，返回满足 `0 <= i < j <= n - 1` 且 `nums[i] * nums[j]` 能被 `k` 整除的下标对 `(i, j)` 的数目。
