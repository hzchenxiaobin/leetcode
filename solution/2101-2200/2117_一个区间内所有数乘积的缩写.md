...
- **题目名称**：一个区间内所有数乘积的缩写
- **链接**：[2117. 一个区间内所有数乘积的缩写](https://leetcode.cn/problems/abbreviating-the-product-of-a-range/)
- **难度**：困难
- **标签**：数学、模拟、对数

## 1. 题目概述

给定正整数 `left` 和 `right`（`left <= right`），计算闭区间 `[left, right]` 中所有整数的**乘积** `P`。由于 `P` 可能极大，需要按以下步骤**缩写**：
