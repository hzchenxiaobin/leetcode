...
- **题目名称**：带阈值的图连通性
- **链接**：[1627. 带阈值的图连通性](https://leetcode.cn/problems/graph-connectivity-with-threshold/)
- **难度**：困难
- **标签**：并查集、数学、数论、图

## 1. 题目概述

有 `n` 座城市，编号 `1` 到 `n`。给定一个整数 `threshold`，按如下规则构造无向图：当且仅当 `x != y` 且 $\gcd(x, y) > \text{threshold}$ 时，城市 `x` 与 `y` 之间有一条边。
