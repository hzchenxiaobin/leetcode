...
- **题目名称**：分配重复整数
- **链接**：[1655. 分配重复整数](https://leetcode.cn/problems/distribute-repeating-integers/)
- **难度**：困难
- **标签**：动态规划、状态压缩、回溯、位运算

## 1. 题目概述

给定长度为 `n` 的整数数组 `nums`（其中至多有 **50 个不同的值**），以及 `m` 个顾客的订单 `quantity`。第 `i` 位顾客**恰好**需要 `quantity[i]` 个**相同**的整数。判断能否把 `nums` 中的整数分配给所有顾客，使每位顾客都拿到指定数量的同一种整数。
