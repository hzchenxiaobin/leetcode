...
- **题目名称**：通过指令创建有序数组
- **链接**：[1649. 通过指令创建有序数组](https://leetcode.cn/problems/create-sorted-array-through-instructions/)
- **难度**：困难
- **标签**：数组、树状数组、线段树

## 1. 题目概述

给定一个整数数组 `instructions`，从左到右依次将其元素插入一个初始为空的有序数组 `nums`。每次插入的**代价**是以下两者的**较小值**：
