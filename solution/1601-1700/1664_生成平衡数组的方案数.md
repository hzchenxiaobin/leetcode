...
- **题目名称**：生成平衡数组的方案数
- **链接**：[1664. 生成平衡数组的方案数](https://leetcode.cn/problems/ways-to-make-a-fair-array/)
- **难度**：中等
- **标签**：数组、前缀和、动态规划

## 1. 题目概述

给定一个整数数组 `nums`，需要选择**恰好一个**下标并删除对应元素（剩余元素下标会因删除而改变）。如果一个数组满足**奇数下标元素之和等于偶数下标元素之和**，则称其为**平衡数组**。返回删除操作后剩余数组成为平衡数组的**方案数**。
