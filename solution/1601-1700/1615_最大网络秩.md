...
- **题目名称**：最大网络秩
- **链接**：[1615. 最大网络秩](https://leetcode.cn/problems/maximal-network-rank/)
- **难度**：中等
- **标签**：图、贪心、枚举

## 1. 题目概述

给定 `n` 座城市与若干双向道路 `roads`，每条 `roads[i] = [ai, bi]` 表示城市 `ai` 与 `bi` 之间有一条双向道路。两座不同城市 `(u, v)` 的**网络秩**定义为与这两座城市直接相连的道路总数；若 `u`、`v` 之间本身有道路，该道路只算**一次**。返回所有不同城市对中**最大的网络秩**。
