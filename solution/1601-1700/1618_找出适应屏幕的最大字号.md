...
- **题目名称**：找出适应屏幕的最大字号
- **链接**：[1618. 找出适应屏幕的最大字号](https://leetcode.cn/problems/maximum-font-to-fit-a-sentence-in-a-screen/)
- **难度**：中等
- **标签**：数组、字符串、二分查找、交互

## 1. 题目概述

给定一个字符串 `text`，要在**宽为 `w`、高为 `h`** 的屏幕上显示该文本（**只能排一排**，不换行）。字体数组 `fonts` 按升序排列，可从中任选一个字号。
