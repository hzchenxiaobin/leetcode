...
- **题目名称**：没有卖出的卖家
- **链接**：[1607. 没有卖出的卖家](https://leetcode.cn/problems/find-sellers-with-no-sales/)
- **难度**：中等
- **标签**：数据库、SQL、`LEFT JOIN`、反连接（anti-join）、`IS NULL`、`NOT EXISTS`、`NOT IN` 的 NULL 陷阱、`YEAR()` vs `BETWEEN` 的可索引性（sargable）、`ORDER BY`

## 1. 题目概述

给定 `Customer`（顾客）、`Orders`（订单）、`Seller`（卖家）三张表，编写 SQL 查询，找出**在 2020 年没有任何销售记录**的卖家的名字。结果按 `seller_name` **升序**返回。
