...
- **题目名称**：找到遗失的ID
- **链接**：[1613. 找到遗失的ID](https://leetcode.cn/problems/find-the-missing-ids/)
- **难度**：中等
- **标签**：SQL、递归 CTE、序列生成、反连接（LEFT JOIN ... IS NULL）/ NOT IN

## 1. 题目概述

给定一张 `Customers` 表，其主键 `customer_id` **不连续**。请编写 SQL 查询，找出所有"遗失的 ID"——即位于 `[MIN(customer_id), MAX(customer_id)]` 区间内、但**不存在于表中**的整数。
