...
- **题目名称**：通过给定词典构造目标字符串的方案数
- **链接**：[1639. 通过给定词典构造目标字符串的方案数](https://leetcode.cn/problems/number-of-ways-to-form-a-target-string-given-a-dictionary/)
- **难度**：困难
- **标签**：动态规划、字符串、计数

## 1. 题目概述

给定一个字符串列表 `words`（所有字符串**长度相同**）和一个目标字符串 `target`。按以下规则从 `words` 中构造 `target`：
