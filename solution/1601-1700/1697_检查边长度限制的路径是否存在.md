...
- **题目名称**：检查边长度限制的路径是否存在
- **链接**：[1697. 检查边长度限制的路径是否存在](https://leetcode.cn/problems/checking-existence-of-edge-length-limited-paths/)
- **难度**：困难
- **标签**：并查集、离线查询、排序

## 1. 题目概述

给你一个 `n` 个点组成的无向图边集 `edgeList`，其中 `edgeList[i] = [ui, vi, disi]` 表示点 `ui` 和点 `vi` 之间有一条长度为 `disi` 的边。两个点之间可能有**多条边**。
