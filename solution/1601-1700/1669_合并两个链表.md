...
- **题目名称**：合并两个链表
- **链接**：[1669. 合并两个链表](https://leetcode.cn/problems/merge-in-between-linked-lists/)
- **难度**：中等
- **标签**：链表、双指针

## 1. 题目概述

给定两个链表 `list1`（`n` 个节点）和 `list2`（`m` 个节点），以及两个整数 `a`、`b`。请将 `list1` 中**下标从 `a` 到 `b`（含两端）**的全部节点删除，并把 `list2` **整体接在被删除节点的位置**，最后返回结果链表的头指针。
