...
- **题目名称**：可达的最大交换次数
- **链接**：[1601. 可达的最大交换次数](https://leetcode.cn/problems/maximum-number-of-achievable-transfer-requests/)
- **难度**：困难
- **标签**：回溯、位运算、枚举

## 1. 题目概述

我们有 `n` 栋楼，编号 `0` 到 `n-1`。给定 `requests` 数组，其中 `requests[i] = [from_i, to_i]` 表示一名员工希望从楼 `from_i` 调换到楼 `to_i`。所有楼的**净变化初始为 0**（即每栋楼的进出人数相等）。
