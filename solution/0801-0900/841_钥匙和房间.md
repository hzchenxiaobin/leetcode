...
- **题目名称**：钥匙和房间
- **链接**：[841. 钥匙和房间](https://leetcode.cn/problems/keys-and-rooms/)
- **难度**：中等
- **标签**：深度优先搜索、广度优先搜索、图、连通性

## 1. 题目概述

有 `n` 个房间，编号 `0` 到 `n-1`。除 **0 号房间**外，每个房间都**锁着**。你从 0 号房间开始，每个房间 `i` 里有一串钥匙 `rooms[i]`，每把钥匙上标着一个房间号，可以打开对应的房间。**返回你是否能进入所有房间**。
