...
- **题目名称**：公交路线
- **链接**：[815. 公交路线](https://leetcode.cn/problems/bus-routes/)
- **难度**：困难
- **标签**：广度优先搜索、图、哈希表

## 1. 题目概述

给你一个数组 `routes` 表示若干公交线路，`routes[i]` 是第 `i` 辆公交的循环路线（反复循环行驶）。例如 `routes = [[1,2,7],[3,6,7]]` 表示 0 号公交走 `1→2→7→1→…`，1 号公交走 `3→6→7→3→…`。
