...
给定一个由 `n` 个节点（编号 `0` 到 `n-1`）组成的**无向**原始图。边列表 `edges[i] = [u_i, v_i, cnt_i]` 表示原图中 `u_i` 与 `v_i` 之间有一条边，并且要把这条边**细分**——即在 `u_i`、`v_i` 之间插入 `cnt_i` 个**新节点**，从而把一条边变成 `cnt_i + 1` 条小段（segment）。
