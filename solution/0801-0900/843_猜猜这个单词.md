...
- **题目名称**：猜猜这个单词
- **链接**：[843. 猜猜这个单词](https://leetcode.cn/problems/guess-the-word/)
- **难度**：困难
- **标签**：交互题、minimax、消除法、字符串

## 1. 题目概述

给定一个由**不同**字符串组成的单词列表 `words`，每个单词长度均为 6，其中一个被选作**秘密单词** `secret`。你拥有一个辅助对象 `Master`，可以调用 `Master.guess(word)` 来猜单词：
