...
- **题目名称**：树中距离之和
- **链接**：[834. 树中距离之和](https://leetcode.cn/problems/sum-of-distances-in-tree/)
- **难度**：困难
- **标签**：树、动态规划、深度优先搜索、换根 DP

## 1. 题目概述

给定一棵无向树（连通无环图），$n$ 个节点编号 $0$ 到 $n-1$，以及边列表 `edges`。返回数组 `ans`，其中 `ans[i]` 表示节点 $i$ 到所有其他节点的**距离之和**（距离 = 两点间路径的边数）。
