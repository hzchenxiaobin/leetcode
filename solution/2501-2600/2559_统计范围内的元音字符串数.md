...
- **题目名称**：统计范围内的元音字符串数
- **链接**：[2559. 统计范围内的元音字符串数](https://leetcode.cn/problems/count-vowel-strings-in-ranges/)
- **难度**：中等
- **标签**：数组、前缀和

## 1. 题目概述

给你一个下标从 **0** 开始的字符串数组 `words` 以及一个二维整数数组 `queries`。每个查询 `queries[i] = [li, ri]` 要求统计 `words` 中下标在 `[li, ri]`（**含两端**）范围内、且**以元音开头并以元音结尾**的字符串数目。返回数组 `ans`，其中 `ans[i]` 为第 `i` 个查询的答案。元音字母为 `'a'、'e'、'i'、'o'、'u'`。
