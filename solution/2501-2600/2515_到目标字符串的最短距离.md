...
- **题目名称**：到目标字符串的最短距离
- **链接**：[2515. 到目标字符串的最短距离](https://leetcode.cn/problems/shortest-distance-to-target-string-in-a-circular-array/)
- **难度**：简单
- **标签**：数组、字符串、环形数组、一次遍历

## 1. 题目概述

给定一个 **0 下标** 的 **环形** 字符串数组 `words` 和一个字符串 `target`。「环形」指数组的尾部与首部相连：`words[i]` 的下一个是 `words[(i + 1) % n]`，上一个是 `words[(i - 1 + n) % n]`，其中 $n = \text{words.length}$。
