...
- **题目名称**：执行操作后的最大 MEX
- **链接**：[2598. 执行操作后的最大 MEX](https://leetcode.cn/problems/smallest-missing-non-negative-integer-after-operations/)
- **难度**：中等
- **标签**：贪心、数组、哈希表、数论

## 1. 题目概述

给定下标从 0 开始的整数数组 `nums` 和整数 `value`。每步操作可对 `nums` 中**任意一个**元素加上或减去 `value`，可执行**任意次**（每个元素可被反复操作，也可不动）。
