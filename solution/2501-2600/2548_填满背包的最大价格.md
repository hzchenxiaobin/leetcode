...
- **题目名称**：填满背包的最大价格
- **链接**：[2548. 填满背包的最大价格](https://leetcode.cn/problems/maximum-price-to-fill-a-bag/)
- **难度**：中等
- **标签**：贪心、数组、排序

## 1. 题目概述

给定二维整数数组 `items`，其中 `items[i] = [price_i, weight_i]` 表示第 $i$ 个物品的**价格**与**重量**。再给定**正**整数容量 `capacity`。
