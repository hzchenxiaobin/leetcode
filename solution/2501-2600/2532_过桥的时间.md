...
- **题目名称**：过桥的时间
- **链接**：[2532. 过桥的时间](https://leetcode.cn/problems/time-to-cross-a-bridge/)
- **难度**：困难
- **标签**：数组、模拟、堆（优先队列）、事件驱动

## 1. 题目概述

$k$ 位工人要把 $n$ 个箱子从**右岸旧仓库**搬到**左岸新仓库**，两仓库隔河相望，仅一座桥可通行。每位工人 $i$ 的耗时由 `time[i] = [right_i, pick_i, left_i, put_i]` 给出：
