...
- **题目名称**：统计可能的树根数目
- **链接**：[2581. 统计可能的树根数目](https://leetcode.cn/problems/count-number-of-possible-root-nodes/)
- **难度**：困难
- **标签**：树、深度优先搜索、动态规划、换根 DP、哈希表

## 1. 题目概述

Alice 有一棵 $n$ 个节点的**无向树**，节点编号 $0$ 到 $n-1$，边由 `edges` 给出。Bob 对树做若干次**猜测**：每次猜 `guesses[j] = [u_j, v_j]` 表示「$u_j$ 是 $v_j$ 的父节点」。
