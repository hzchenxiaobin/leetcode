...
- **题目名称**：连接后等于目标字符串的字符串对
- **链接**：[2023. 连接后等于目标字符串的字符串对](https://leetcode.cn/problems/number-of-pairs-of-strings-with-concatenation-equal-to-target/)
- **难度**：中等
- **标签**：哈希表、字符串、计数

## 1. 题目概述

给定一个**数字**字符串数组 `nums` 和一个**数字**字符串 `target`，返回满足 `nums[i] + nums[j]`（字符串连接）等于 `target` 且 `i != j` 的下标对 `(i, j)` 的数目。
