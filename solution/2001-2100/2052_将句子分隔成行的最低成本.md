...
- **题目名称**：将句子分隔成行的最低成本
- **链接**：[2052. 将句子分隔成行的最低成本](https://leetcode.cn/problems/minimum-cost-to-separate-sentence-into-rows)
- **难度**：中等
- **标签**：字符串、动态规划、记忆化搜索

## 1. 题目概述

给定一个由空格分隔的单词组成的字符串 `sentence` 和一个整数 `k`。任务是将 `sentence` 分成**多行**，每行中的字符数**最多**为 `k`。可以假设 `sentence` 不以空格开头或结尾，且 `sentence` 中的单词由单个空格分隔。
