...
- **题目名称**：前往目标城市的最小费用
- **链接**：[2093. 前往目标城市的最小费用](https://leetcode.cn/problems/minimum-cost-to-reach-city-with-discounts/)
- **难度**：中等
- **标签**：图、最短路径、Dijkstra、堆优先队列、分层状态

## 1. 题目概述

有 $n$ 个城市（编号 $0$ 到 $n-1$），由若干双向高速公路连接。`highways[i] = [city1, city2, toll]` 表示一条连接 `city1` 与 `city2`、通行费为 `toll` 的高速公路。除 `highways` 中给出的公路外，城市间不连通。
