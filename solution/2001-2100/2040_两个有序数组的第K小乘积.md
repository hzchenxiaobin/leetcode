...
- **题目名称**：两个有序数组的第 K 小乘积
- **链接**：[2040. 两个有序数组的第 K 小乘积](https://leetcode.cn/problems/kth-smallest-product-of-two-sorted-arrays/)
- **难度**：困难
- **标签**：数组、二分查找、二分答案

## 1. 题目概述

给定两个**从小到大排好序**的整数数组 `nums1` 和 `nums2` 以及一个整数 `k`，返回所有 `nums1[i] * nums2[j]` 乘积中**第 `k` 小**的那个（`k` 从 1 开始）。
