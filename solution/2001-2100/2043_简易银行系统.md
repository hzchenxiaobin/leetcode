...
- **题目名称**：简易银行系统
- **链接**：[2043. 简易银行系统](https://leetcode.cn/problems/simple-bank-system/)
- **难度**：中等
- **标签**：设计、数组、模拟

## 1. 题目概述

为一个有 `n` 个账户的银行设计程序，自动化执行**转账、存款、取款**三类交易。账户编号从 `1` 到 `n`，初始余额存放在下标从 **0** 开始的数组 `balance` 中，第 `(i+1)` 个账户的初始余额是 `balance[i]`。
