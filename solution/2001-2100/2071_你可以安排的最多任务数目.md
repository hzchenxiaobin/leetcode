...
- **题目名称**：你可以安排的最多任务数目
- **链接**：[2071. 你可以安排的最多任务数目](https://leetcode.cn/problems/maximum-number-of-tasks-you-can-assign/)
- **难度**：困难
- **标签**：数组、二分查找、贪心、有序集合

## 1. 题目概述

给定 `tasks`（`n` 个任务的力量需求）和 `workers`（`m` 个工人的力量值）。每个工人**至多完成一个**任务，且需 `workers[j] >= tasks[i]`。另有 `pills` 片药丸，每片可让**一个**工人的力量值增加 `strength`，每个工人**至多服一片**。返回**最多**能完成的任务数。
