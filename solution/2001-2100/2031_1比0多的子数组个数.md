...
- **题目名称**：1 比 0 多的子数组个数
- **链接**：[2031. 1 比 0 多的子数组个数](https://leetcode.cn/problems/count-subarrays-with-more-ones-than-zeros/)
- **难度**：中等
- **标签**：树状数组、前缀和、分治、归并排序

## 1. 题目概述

给你一个只包含 `0` 和 `1` 的数组 `nums`，请返回 `1` 的数量**大于** `0` 的数量的**子数组**的个数。由于答案可能很大，请返回答案对 $10^9 + 7$ **取余**的结果。
