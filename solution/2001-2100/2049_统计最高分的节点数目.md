...
- **题目名称**：统计最高分的节点数目
- **链接**：[2049. 统计最高分的节点数目](https://leetcode.cn/problems/count-nodes-with-the-highest-score/)
- **难度**：中等
- **标签**：树、深度优先搜索、二叉树

## 1. 题目概述

给你一棵根节点为 `0` 的**二叉树**，共 `n` 个节点（编号 `0` 到 `n-1`）。给定 `parents`，其中 `parents[i]` 是节点 `i` 的父节点（`parents[0] == -1`）。
