...
- **题目名称**：网络空闲的时刻
- **链接**：[2039. 网络空闲的时刻](https://leetcode.cn/problems/the-time-when-the-network-becomes-idle/)
- **难度**：中等
- **标签**：图、BFS、最短路径、数学

## 1. 题目概述

有 `n` 个服务器（编号 `0` 到 `n-1`），通过无向边 `edges` 连成一张**连通**图。每条边传输信息耗时 **1 秒**，一秒内可传输任意数目信息。编号 `0` 的服务器是**主服务器**，其余为**数据服务器**。
