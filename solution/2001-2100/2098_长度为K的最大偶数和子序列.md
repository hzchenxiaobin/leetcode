...
- **题目名称**：长度为 K 的最大偶数和子序列
- **链接**：[2098. 长度为 K 的最大偶数和子序列](https://leetcode.cn/problems/subsequence-of-size-k-with-the-largest-even-sum/)
- **难度**：中等
- **标签**：贪心、排序、数组

## 1. 题目概述

给定一个整数数组 `nums` 和一个整数 `k`，从 `nums` 中选出一个长度为 `k` 的**子序列**，使其元素之和为**最大的偶数**。如果不存在这样的子序列，返回 `-1`。
