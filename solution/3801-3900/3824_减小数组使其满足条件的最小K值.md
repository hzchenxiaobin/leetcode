...
- **题目名称**：减小数组使其满足条件的最小K值
- **链接**：[3824. 减小数组使其满足条件的最小K值](https://leetcode.cn/problems/minimum-k-to-reduce-array-within-limit/)
- **难度**：中等
- **标签**：数组、二分查找、数学

## 1. 题目概述

给你一个**正**整数数组 `nums`。对于正整数 $k$，定义 $\text{nonPositive}(nums, k)$ 为使 `nums` 的每个元素都变为**非正数**所需的最小操作次数——一次操作可以选择一个下标 $i$ 并将 $nums[i]$ 减少 $k$。
