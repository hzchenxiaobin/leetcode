...
- **题目名称**：预算下的最大总容量
- **链接**：[3814. 预算下的最大总容量](https://leetcode.cn/problems/maximum-capacity-within-budget/)
- **难度**：中等
- **标签**：数组、排序、双指针、二分查找

## 1. 题目概述

给你两个长度为 $n$ 的整数数组 `costs` 和 `capacity`，其中 `costs[i]` 表示第 $i$ 台机器的**购买成本**，`capacity[i]` 表示其**性能容量**。同时给定一个整数 `budget`。
...
**约束条件**：$1 \le n \le 10^5$，$1 \le costs[i], capacity[i] \le 10^5$，$1 \le budget \le 2 \times 10^5$。
