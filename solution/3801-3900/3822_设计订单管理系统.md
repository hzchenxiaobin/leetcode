...
- **题目名称**：设计订单管理系统
- **链接**：[3822. Design Order Management System](https://leetcode.cn/problems/design-order-management-system/)
- **难度**：中等
- **标签**：设计、哈希表
...
设计一个订单管理系统，实时管理买单与卖单。每笔订单由三元组 `(orderId, orderType, price)` 描述，`orderType` 为 `"buy"`（买单）或 `"sell"`（卖单）。实现 `OrderManagementSystem` 类：
