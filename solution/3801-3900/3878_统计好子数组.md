...
- **题目名称**：统计好子数组
- **链接**：[3878. 统计好子数组](https://leetcode.cn/problems/count-good-subarrays/)
- **难度**：困难
- **标签**：栈、位运算、数组、单调栈

## 1. 题目概述

给你一个整数数组 `nums`。如果一段**子数组**中所有元素的**按位或**等于该子数组中**至少出现一次**的元素，则称其为**好子数组**。返回 `nums` 中好子数组的数量。
...
**约束条件**：$1 \le n \le 10^5$，$0 \le \text{nums}[i] \le 10^9$。
