...
- **题目名称**：查询树上回文路径
- **链接**：[3841. 查询树上回文路径](https://leetcode.cn/problems/palindromic-path-queries-in-a-tree/)
- **难度**：困难
- **标签**：位运算、树、深度优先搜索、最近公共祖先、树状数组

## 1. 题目概述

给你一棵 $n$ 个节点的无向树（节点编号 $0 \sim n-1$），节点 $i$ 上写着小写字母 $s[i]$。再给一个操作数组 `queries`，每个操作是以下两种之一：
