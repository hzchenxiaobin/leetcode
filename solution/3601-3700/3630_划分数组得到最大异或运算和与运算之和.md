...
- **题目名称**：划分数组得到最大异或运算和与运算之和
- **链接**：[3630. Partition Array for Maximum XOR and AND](https://leetcode.cn/problems/partition-array-for-maximum-xor-and-and/)
- **难度**：困难
- **标签**：贪心、位运算、数组、数学、枚举

## 1. 题目概述

给你一个整数数组 `nums`。将数组划分为**三**个（可以为空）子序列 `A`、`B`、`C`，使得 `nums` 中每个元素**恰好属于一个**子序列。目标是最大化：
