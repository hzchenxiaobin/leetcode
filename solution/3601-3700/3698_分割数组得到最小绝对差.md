...
- **题目名称**：分割数组得到最小绝对差
- **链接**：[3698. Split Array With Minimum Difference](https://leetcode.cn/problems/split-array-with-minimum-difference/)
- **难度**：中等
- **标签**：数组、前缀和

## 1. 题目概述

给你一个整数数组 `nums`。将数组**恰好**分成两个子数组 `left` 和 `right`，使得 `left` **严格递增**、`right` **严格递减**。
