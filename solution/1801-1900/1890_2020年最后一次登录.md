...
- **题目名称**：2020年最后一次登录
- **链接**：[1890. 2020年最后一次登录](https://leetcode.cn/problems/the-latest-login-in-2020/)
- **难度**：简单
- **标签**：SQL、`GROUP BY`、`MAX()`、`WHERE`、`YEAR()` 函数、分组求最值

## 1. 题目概述

给定 `Logins` 表（登录记录，含用户 ID 和登录时间），编写 SQL 查询获取**在 2020 年登录过的所有用户**的 2020 年内**最后一次登录时间**。结果集**不包含** 2020 年没有登录过的用户，可以按任意顺序排列。
