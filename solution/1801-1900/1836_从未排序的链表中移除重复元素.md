...
- **题目名称**：从未排序的链表中移除重复元素
- **链接**：[1836. 从未排序的链表中移除重复元素](https://leetcode.cn/problems/remove-duplicates-from-an-unsorted-linked-list/)
- **难度**：中等
- **标签**：链表、哈希表

## 1. 题目概述

给定一个**未排序**的单链表的头节点 `head`，删除链表中所有**出现超过一次**的值的节点——即若某个值在链表里出现 $\geq 2$ 次，则该值对应的**所有节点**都要删除，一个不留。返回结果链表的头节点。
