...
- **题目名称**：可移除字符的最大数目
- **链接**：[1898. 可移除字符的最大数目](https://leetcode.cn/problems/maximum-number-of-removable-characters/)
- **难度**：中等
- **标签**：数组、二分查找、双指针、字符串

## 1. 题目概述

给你两个字符串 `s` 和 `p`，其中 `p` 是 `s` 的一个**子序列**。同时给你一个元素**互不相同**且下标从 0 开始的整数数组 `removable`，它是 `s` 中下标的一个子集。
