...
- **题目名称**：向下取整数对和
- **链接**：[1862. 向下取整数对和](https://leetcode.cn/problems/sum-of-floored-pairs/)
- **难度**：困难
- **标签**：数组、值域计数、前缀和、调和级数、数学

## 1. 题目概述

给定整数数组 `nums`，返回所有下标对 `0 <= i, j < nums.length` 的 $\lfloor \text{nums}[i] / \text{nums}[j] \rfloor$ 之和。由于答案可能很大，返回对 $10^9 + 7$ 取余的结果。函数 `floor()` 返回输入数字的整数部分（向下取整）。
