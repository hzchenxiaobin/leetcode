...
- **题目名称**：到目标元素的最小距离
- **链接**：[1848. 到目标元素的最小距离](https://leetcode.cn/problems/minimum-distance-to-the-target-element/)
- **难度**：简单
- **标签**：数组、一次遍历

## 1. 题目概述

给你一个整数数组 `nums`（**下标从 0 开始**）以及两个整数 `target` 和 `start`，请你找出一个下标 `i`，满足 $\text{nums}[i] == \text{target}$ 且 $\text{abs}(i - \text{start})$ **最小化**。返回 $\text{abs}(i - \text{start})$。
