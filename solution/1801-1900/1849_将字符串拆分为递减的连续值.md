...
- **题目名称**：将字符串拆分为递减的连续值
- **链接**：[1849. 将字符串拆分为递减的连续值](https://leetcode.cn/problems/splitting-a-string-into-descending-consecutive-values/)
- **难度**：中等
- **标签**：字符串、回溯、剪枝

## 1. 题目概述

给定一个仅由数字组成的字符串 `s`，判断能否将其拆分成**两个或更多非空子字符串**，使各子字符串的**数值**按**降序**排列，且每两个相邻子字符串的数值之**差**等于 `1`。
