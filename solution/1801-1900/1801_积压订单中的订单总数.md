...
- **题目名称**：积压订单中的订单总数
- **链接**：[1801. Number of Orders in the Backlog](https://leetcode.cn/problems/number-of-orders-in-the-backlog/)
- **难度**：中等
- **标签**：堆、贪心、模拟、优先队列

## 1. 题目概述

给定一个二维整数数组 `orders`，其中 `orders[i] = [pricei, amounti, orderTypei]` 表示一批共 `amounti` 笔、价格为 `pricei`、类型为 `orderTypei` 的订单：
