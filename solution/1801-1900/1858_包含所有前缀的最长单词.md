...
- **题目名称**：包含所有前缀的最长单词
- **链接**：[1858. 包含所有前缀的最长单词](https://leetcode.cn/problems/longest-word-with-all-prefixes/)
- **难度**：中等
- **标签**：字典树、深度优先搜索、哈希表、排序

## 1. 题目概述

给定一个字符串数组 `words`，找出其中**最长**的单词——该单词的**每一个前缀**（从长度 1 到自身长度的所有前缀）都必须出现在 `words` 中。若存在多个等长候选，返回**字典序最小**的那个；若无解，返回空字符串 `""`。
