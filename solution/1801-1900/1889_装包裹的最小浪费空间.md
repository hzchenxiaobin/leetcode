...
- **题目名称**：装包裹的最小浪费空间
- **链接**：[1889. 装包裹的最小浪费空间](https://leetcode.cn/problems/minimum-space-wasted-from-packaging/)
- **难度**：困难
- **标签**：数组、二分查找、前缀和、排序

## 1. 题目概述

给定 `n` 个包裹（尺寸数组 `packages`）和 `m` 个供应商（每个供应商提供一组**不同尺寸**的箱子，每种尺寸数量无限）。**每个箱子只能装一个包裹**，且包裹尺寸需 **小于等于** 箱子尺寸。
