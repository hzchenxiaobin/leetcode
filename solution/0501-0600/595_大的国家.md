...
- **题目名称**：大的国家
- **链接**：[595. 大的国家](https://leetcode.cn/problems/big-countries/)
- **难度**：简单
- **标签**：SQL、WHERE、OR、数值比较

## 1. 题目概述

给定 `World` 表，每行记录一个国家的 `name`（名称）、`continent`（所属大陆）、`area`（面积）、`population`（人口）和 `gdp`（国内生产总值）。如果一个国家满足下列**任一条件**，则认为该国是「大国」：
