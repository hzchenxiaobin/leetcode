...
- **题目名称**：查询回答率最高的问题
- **链接**：[578. 查询回答率最高的问题](https://leetcode.cn/problems/get-highest-answer-rate-question/)
- **难度**：中等
...
给定 `SurveyLog` 表，每行记录一次问卷日志的 `action`（展示 `show` / 回答 `answer` / 跳过 `skip`）、对应的问题编号 `question_id` 及回答编号 `answer_id` 等。编写 SQL 查询，返回**回答率最高**的 `question_id`。
