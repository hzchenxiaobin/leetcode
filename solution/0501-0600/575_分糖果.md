...
- **题目名称**：分糖果
- **链接**：[575. 分糖果](https://leetcode.cn/problems/distribute-candies/)
- **难度**：简单
- **标签**：数组、哈希表、贪心

## 1. 题目概述

Alice 有 $n$ 颗糖果，数组 `candyType` 中 `candyType[i]` 表示第 $i$ 颗糖果的**类型**。她患了糖尿病，医生只允许她吃 $n/2$ 颗。她希望吃到**尽可能多不同类型**的糖果。返回她最多能吃到的不同类型数量。
