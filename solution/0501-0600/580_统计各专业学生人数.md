...
- **题目名称**：统计各专业学生人数
- **链接**：[580. Count Student Number in Departments](https://leetcode.cn/problems/count-student-number-in-departments/)
- **难度**：中等
...
给定 `Department` 表（专业，含 `dept_id` 与 `dept_name`）与 `Student` 表（学生，含 `student_id`、`student_name`、`gender` 及所属 `dept_id`）。编写 SQL 查询，统计**每个专业的学生人数**，输出 `dept_name` 与 `student_number` 两列。
