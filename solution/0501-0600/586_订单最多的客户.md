...
- **题目名称**：订单最多的客户
- **链接**：[586. 订单最多的客户](https://leetcode.cn/problems/customer-placing-the-largest-number-of-orders/)
- **难度**：简单
- **标签**：SQL、聚合、GROUP BY、COUNT、ORDER BY、LIMIT

## 1. 题目概述

给定 `Orders` 表，每行记录一个订单的编号 `order_number` 及下单客户的编号 `customer_number`。编写 SQL 查询，返回**下了最多订单**的客户的 `customer_number`。
