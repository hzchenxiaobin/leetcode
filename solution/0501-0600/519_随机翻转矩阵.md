...
- **题目名称**：随机翻转矩阵
- **链接**：[519. 随机翻转矩阵](https://leetcode.cn/problems/random-flip-matrix/)
- **难度**：中等
- **标签**：概率与随机、哈希表、Fisher-Yates 洗牌

## 1. 题目概述

给你一个 `m x n` 的二元矩阵 `matrix`，且所有值被初始化为 `0`。请设计一个算法，**随机选取**一个满足 `matrix[i][j] == 0` 的下标 `(i, j)`，并将它的值变为 `1`。所有满足 `matrix[i][j] == 0` 的下标 `(i, j)` 被选取的概率应当**均等**。
