...
- **题目名称**：两个列表的最小索引总和
- **链接**：[599. 两个列表的最小索引总和](https://leetcode.cn/problems/minimum-index-sum-of-two-lists/)
- **难度**：简单
- **标签**：数组、哈希表

## 1. 题目概述

给定两个字符串数组 `list1` 和 `list2`，找到**索引和最小的公共字符串**。**公共字符串**是同时出现在 `list1` 和 `list2` 中的字符串。若某字符串在 `list1[i]` 和 `list2[j]` 中出现，则其索引和为 $i + j$。返回所有具有**最小索引和**的公共字符串，顺序任意。
