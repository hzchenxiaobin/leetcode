...
- **题目名称**：范围内整数的最大得分
- **链接**：[3281. Maximize Score of Numbers in Ranges](https://leetcode.cn/problems/maximize-score-of-numbers-in-ranges/)
- **难度**：中等
- **标签**：贪心、数组、二分查找、排序

## 1. 题目概述

给你一个整数数组 `start` 和一个整数 `d`，代表 $n$ 个**区间** $[\text{start}[i],\ \text{start}[i] + d]$。
