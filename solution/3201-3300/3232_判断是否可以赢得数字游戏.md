...
- **题目名称**：判断是否可以赢得数字游戏
- **链接**：[3232. 判断是否可以赢得数字游戏](https://leetcode.cn/problems/find-if-digit-game-can-be-won/)
- **难度**：简单
- **标签**：数组、数学、脑筋急转弯

## 1. 题目概述

给你一个**正整数**数组 `nums`。Alice 和 Bob 正在玩游戏：Alice 可以从 `nums` 中选择**所有个位数**或**所有两位数**，剩余的数字归 Bob 所有。如果 Alice 所选数字之和**严格大于** Bob 的数字之和，则 Alice 获胜。
