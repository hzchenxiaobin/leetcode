...
- **题目名称**：两两交换链表中的节点
- **链接**：[24. 两两交换链表中的节点](https://leetcode.cn/problems/swap-nodes-in-pairs/)
- **难度**：中等
- **标签**：链表

## 1. 题目概述

给定一个单链表，**两两交换**其中**相邻的节点**，并返回交换后链表的头节点。必须实际**修改节点指针**（不能只交换值）。如果节点数为奇数，最后一个节点保持原位。
...
> 💡 这是链表**指针操作**的经典题，和 [反转链表（206）](../../week1/day4/反转链表.md)、[K 个一组翻转（25）](../../week5/day7/K个一组翻转链表.md) 一脉相承。核心是**哑节点 + 三指针**：用 `prev` 指向待交换对的前驱，每次交换它后面的两个节点 `a` 和 `b`，把 `prev→a→b→...` 变成 `prev→b→a→...`。K=2 的特例就是本题，掌握它再推广到 K 个一组翻转。
...
本题是 [K 个一组翻转链表（25）](../../week5/day7/K个一组翻转链表.md) 在 `K=2` 时的特例。25 题中每 K 个一组翻转，套路完全一致：
...
- [206. 反转链表](https://leetcode.cn/problems/reverse-linked-list/)（[题解](../../week1/day4/反转链表.md)）：链表指针操作基础，三指针掉头
- [25. K 个一组翻转链表](https://leetcode.cn/problems/reverse-nodes-in-k-group/)（[题解](../../week5/day7/K个一组翻转链表.md)）：本题的 K 通用版，分段翻转
- [92. 反转链表 II](https://leetcode.cn/problems/reverse-linked-list-ii/)（[题解](../day4/反转链表 II.md)）：反转指定区间，哑节点 + 局部翻转
- [21. 合并两个有序链表](https://leetcode.cn/problems/merge-two-sorted-lists/)（[题解](../../week2/day4/合并两个有序链表.md)）：哑节点统一头处理的经典用法
- [445. 两数相加 II](https://leetcode.cn/problems/add-two-numbers-ii/)（[题解](../day5/两数相加 II.md)）：链表 + 栈/反转，综合指针操作
