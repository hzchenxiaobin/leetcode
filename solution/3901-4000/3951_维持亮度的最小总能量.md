...
- **题目名称**：维持亮度的最小总能量
- **链接**：[3951. 维持亮度的最小总能量](https://leetcode.cn/problems/minimum-energy-to-maintain-brightness/)
- **难度**：中等
- **标签**：数组、排序、区间合并

## 1. 题目概述

$n$ 个灯泡排成一排，下标 $0 \sim n-1$。再给定整数 `brightness`（下记 $B$）与二维整数数组 `intervals`，其中 `intervals[i] = [start_i, end_i]` 表示一个**闭**时间区间——该区间内的每个单位时间都必须满足照明要求。
