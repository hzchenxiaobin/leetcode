# 统计从两个字符串形成目标字符串的不同方案数

- **题目名称**：统计从两个字符串形成目标字符串的不同方案数
- **链接**：[3981. 统计从两个字符串形成目标字符串的不同方案数](https://leetcode.cn/problems/count-distinct-ways-to-form-target-from-two-strings/)
- **难度**：困难
- **标签**：字符串、动态规划、前缀和

## 1. 题目概述

给你三个字符串 `word1`、`word2` 和 `target`。你的任务是计算从 `word1` 和 `word2` 中**选择字符以形成 `target`** 的方案数，需满足：
