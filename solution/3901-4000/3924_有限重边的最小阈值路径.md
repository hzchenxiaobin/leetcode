...
- **题目名称**：有限重边的最小阈值路径
- **链接**：[3924. Minimum Threshold Path With Limited Heavy Edges](https://leetcode.cn/problems/minimum-threshold-path-with-limited-heavy-edges/)
- **难度**：困难
- **标签**：二分查找、图、广度优先搜索、最短路（0-1 BFS）

## 1. 题目概述

给你一个有 $n$ 个节点的**无向加权图**，节点编号 $0$ 到 $n-1$，边由二维数组 `edges` 给出，`edges[i] = [u_i, v_i, w_i]` 表示 $u_i$ 与 $v_i$ 之间有一条权为 $w_i$ 的无向边。
