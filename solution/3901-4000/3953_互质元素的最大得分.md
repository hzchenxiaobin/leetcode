...
- **题目名称**：互质元素的最大得分
- **链接**：[3953. 互质元素的最大得分](https://leetcode.cn/problems/maximum-score-with-co-prime-element/)
- **难度**：困难
- **标签**：数组、哈希表、数学、数论、枚举、容斥原理

## 1. 题目概述

给你一个长度为 $n$ 的整数数组 `nums` 和一个整数 `maxVal`。你可以将 `nums` 中的任意元素更改为**小于或等于 `maxVal` 的任意正整数**，每次这样的更改代价为 $1$。
