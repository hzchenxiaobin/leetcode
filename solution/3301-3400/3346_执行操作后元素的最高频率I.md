...
- **题目名称**：执行操作后元素的最高频率 I
- **链接**：[3346. 执行操作后元素的最高频率 I](https://leetcode.cn/problems/maximum-frequency-of-an-element-after-performing-operations-i/)
- **难度**：中等
- **标签**：数组、排序、二分查找、滑动窗口
...
给你一个整数数组 `nums` 和两个整数 `k` 和 `numOperations`。你必须对 `nums` 执行操作 `numOperations` 次，每次操作中你可以：
