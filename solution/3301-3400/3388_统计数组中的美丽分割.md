...
- **题目名称**：统计数组中的美丽分割
- **链接**：[3388. 统计数组中的美丽分割](https://leetcode.cn/problems/count-beautiful-splits-in-an-array/)
- **难度**：中等
...
给你一个整数数组 `nums`。把它切成三段**非空连续子数组** `nums1`、`nums2`、`nums3`（按顺序连接恰好还原 `nums`），若满足 **`nums1` 是 `nums2` 的前缀**，或 **`nums2` 是 `nums3` 的前缀**，则称这是一个**美丽分割**。返回美丽分割的数目。
