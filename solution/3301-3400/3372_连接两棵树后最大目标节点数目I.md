...
- **题目名称**：连接两棵树后最大目标节点数目 I
- **链接**：[3372. 连接两棵树后最大目标节点数目 I](https://leetcode.cn/problems/maximize-the-number-of-target-nodes-after-connecting-trees-i/)
- **难度**：中等
- **标签**：树、深度优先搜索、广度优先搜索
...
给出两棵**无向树**，分别有 $n$ 和 $m$ 个节点，节点编号分别为 $[0, n-1]$ 与 $[0, m-1]$。二维整数数组 `edges1`（长 $n-1$）与 `edges2`（长 $m-1$）描述两棵树的边，另有整数 $k$。
