...
- **题目名称**：词典中最长的单词
- **链接**：[720. 词典中最长的单词](https://leetcode.cn/problems/longest-word-in-dictionary/)
- **难度**：中等
- **标签**：字典树、数组、哈希表、排序、深度优先搜索

## 1. 题目概述

给出一个字符串数组 `words` 作为英文词典，返回**最长**的单词——该单词能由词典中其他单词**每次加一个字符**逐步构建出来。若存在多个等长候选，返回**字典序最小**的那个；若无解，返回空字符串 `""`。
