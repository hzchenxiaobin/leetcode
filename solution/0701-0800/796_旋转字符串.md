...
- **题目名称**：旋转字符串
- **链接**：[796. 旋转字符串](https://leetcode.cn/problems/rotate-string/)
- **难度**：简单
- **标签**：字符串、子串判定、KMP

## 1. 题目概述

给定两个字符串 `s` 和 `goal`。`s` 的**旋转操作**定义为将 `s` 最左边的字符移动到最右边（可执行任意次，包括 0 次）。问：经过若干次旋转后，`s` 能否变成 `goal`？能则返回 `true`，否则 `false`。
