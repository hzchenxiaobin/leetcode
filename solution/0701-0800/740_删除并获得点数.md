...
- **题目名称**：删除并获得点数
- **链接**：[740. 删除并获得点数](https://leetcode.cn/problems/delete-and-earn/)
- **难度**：中等
- **标签**：数组、哈希表、动态规划

## 1. 题目概述

给你一个整数数组 `nums`，你可以对它进行一系列操作：每次操作**任意**选择一个 `nums[i]`，删除它并获得 `nums[i]` 个点数。但是，删除 `nums[i]` 之后，**所有等于 `nums[i] - 1` 和 `nums[i] + 1` 的元素也会被一并删除**。返回你能通过这些操作获得的最大点数。
