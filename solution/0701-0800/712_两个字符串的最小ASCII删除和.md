...
- **链接**：[712. 两个字符串的最小ASCII删除和](https://leetcode.cn/problems/minimum-ascii-delete-sum-for-two-strings/)
- **难度**：中等
- **标签**：动态规划、二维 DP、字符串、LCS

## 1. 题目概述

给定两个字符串 `s1` 和 `s2`，返回使两个字符串**相等**所需删除字符的 **ASCII 值的最小和**。每一步可以删除 `s1` 或 `s2` 中的一个字符，并把该字符的 ASCII 值累加进总和。
