...
- **题目名称**：网络延迟时间
- **链接**：[743. 网络延迟时间](https://leetcode.cn/problems/network-delay-time/)
- **难度**：中等
- **标签**：图、最短路径、Dijkstra、堆/优先队列

## 1. 题目概述

有 `n` 个网络节点，标记为 `1` 到 `n`。给定边列表 `times`，其中 `times[i] = (u, v, w)` 表示一条从 `u` 到 `v` 的**有向**边，信号沿该边传递耗时 `w`。现从节点 `k` 发出一个信号，求**所有节点都收到信号**所需的最少时间；若无法让所有节点收到，返回 `-1`。
