...
给定两个字符串 `order` 和 `s`。`order` 的所有字母都是**唯一**的，并且预先按照某种**自定义顺序**排序。要求对 `s` 的字符进行置换，使其与 `order` 的排序相匹配：若 `order` 中字符 `x` 出现在字符 `y` 之前，则排列结果中 `x` 也应出现在 `y` 之前。返回满足该性质的 `s` 的**任意一种**排列。
