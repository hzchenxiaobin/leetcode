...
- **题目名称**：最大栈
- **链接**：[716. 最大栈](https://leetcode.cn/problems/max-stack/)
- **难度**：困难
- **标签**：栈、设计、堆、懒删除

## 1. 题目概述

设计一个**最大栈** `MaxStack`，在普通栈 `push` / `pop` / `top` 的基础上，额外支持**查询**并**弹出**栈中最大元素。其中 `popMax` 在最大值有多个时，要弹出**最靠近栈顶**的那一个（即最近压入的那个）。
