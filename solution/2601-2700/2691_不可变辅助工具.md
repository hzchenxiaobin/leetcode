...
- **题目名称**：不可变辅助工具
- **链接**：[2691. Immutability Helper](https://leetcode.cn/problems/immutability-helper/)
- **难度**：困难
...
给定一个不可变的 JSON 对象 `obj` 与一个**变换函数数组** `mutators`。每个变换函数 `f` 接收一个**代理** `proxy`，可以像操作普通对象一样对 `proxy` 读属性、写属性（包括嵌套读写）：
