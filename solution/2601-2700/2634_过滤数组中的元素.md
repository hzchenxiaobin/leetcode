...
- **题目名称**：过滤数组中的元素
- **链接**：[2634. 过滤数组中的元素](https://leetcode.cn/problems/filter-elements-from-array/)
- **难度**：简单
- **标签**：数组、高阶函数、谓词筛选、真值判定

## 1. 题目概述

给定一个整数数组 `arr` 和一个过滤函数 `fn`，返回过滤后的数组 `filteredArr`。`fn` 接收一或两个参数：`arr[i]`（元素值）与 `i`（下标）。`filteredArr` 只保留使 `fn(arr[i], i)` 返回值为**真值（truthy）** 的元素——**真值** 指 `Boolean(value)` 返回 `true` 的值。**不得使用内置的 `Array.filter`。**
