...
- **题目名称**：转换数组中的每个元素
- **链接**：[2635. 转换数组中的每个元素](https://leetcode.cn/problems/apply-transform-over-each-element-in-array/)
- **难度**：简单
- **标签**：数组、函数式编程、高阶函数

## 1. 题目概述

给定一个整数数组 `arr` 和一个映射函数 `fn`，返回一个**新数组**，其中每个元素是 `fn` 作用于 `arr` 对应元素的结果。**不得使用内置的 `Array.prototype.map` 方法。**
