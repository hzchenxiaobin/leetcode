...
- **题目名称**：数组归约运算
- **链接**：[2626. 数组归约运算](https://leetcode.cn/problems/array-reduce-transformation/)
- **难度**：简单
- **标签**：数组、函数式编程、高阶函数

## 1. 题目概述

给定一个整数数组 `nums`、一个 reducer 函数 `fn` 和一个初始值 `init`，返回通过依次对数组的每个元素执行 `fn` 得到的最终结果：`val = fn(init, nums[0])`，`val = fn(val, nums[1])`，……，处理完每个元素后返回 `val`。若数组长度为 0，直接返回 `init`。**不得使用内置的 `Array.reduce`。**
