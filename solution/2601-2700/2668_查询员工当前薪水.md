...
- **题目名称**：查询员工当前薪水
- **链接**：[2668. 查询员工当前薪水 🔒](https://leetcode.cn/problems/find-latest-salaries/)
- **难度**：简单
...
给定一张 `Salary` 表，记录员工的薪水信息。由于历史调薪，**同一员工可能出现多行薪水记录**（不同 `salary` 值）。请编写 SQL 查询，找出**每位员工当前的（最新）薪水**，使结果中每位员工恰好出现一行。
