...
- **题目名称**：最小化旅行的价格总和
- **链接**：[2646. 最小化旅行的价格总和](https://leetcode.cn/problems/minimize-the-total-price-of-the-trips/)
- **难度**：困难
- **标签**：树、深度优先搜索、图、动态规划

## 1. 题目概述

现有一棵**无向、无根的树**，树中有 `n` 个节点，按 `0` 到 `n - 1` 编号。给定整数 `n`、长度为 `n - 1` 的二维整数数组 `edges`（`edges[i] = [a_i, b_i]` 表示节点 `a_i` 与 `b_i` 间有一条边）。
