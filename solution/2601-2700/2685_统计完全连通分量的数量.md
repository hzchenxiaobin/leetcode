...
- **题目名称**：统计完全连通分量的数量
- **链接**：[2685. 统计完全连通分量的数量](https://leetcode.cn/problems/count-the-number-of-complete-components/)
- **难度**：中等
- **标签**：并查集、深度优先搜索、广度优先搜索、图

## 1. 题目概述

给定整数 `n`，一个包含 `n` 个顶点的**无向图**，顶点编号 `0` 到 `n-1`，边由 `edges` 给出，`edges[i] = [ai, bi]` 表示 `ai` 与 `bi` 间有一条**无向边**。
